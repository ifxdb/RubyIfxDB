...
#include "ruby_ifx_db_cli.h"
#include "ruby_ifx_db.h"
#include <ctype.h>
...
static void _ruby_ifx_db_check_sql_errors( void *conn_or_stmt, int resourceType, SQLHANDLE handle, SQLSMALLINT hType,
                                           int rc, int cpy_to_global, SQLPOINTER ret_str, SQLSMALLINT *ret_str_len, int API,
                                           SQLSMALLINT recno, int release_gil );
static VALUE _ruby_ifx_db_assign_options( void* handle, int type, long opt_key, VALUE data, VALUE *error );
static void _ruby_ifx_db_clear_conn_err_cache();
static void _ruby_ifx_db_clear_stmt_err_cache();
static int  _ruby_ifx_db_set_decfloat_rounding_mode_client(SQLHANDLE hdbc);
static char *_ruby_ifx_db_instance_name;
...
*/
typedef struct _ifx_db_connect_helper_args_struct {
...
/*
   Structure to hold the data necessary for the function ifx_db_result to retrieve the corresponding data
*/
typedef struct _ifx_db_result_args_struct {
  stmt_handle  *stmt_res;
  VALUE        column;
  VALUE        *error;
  VALUE 	   return_value;
...
*/
typedef struct _ifx_db_fetch_helper_struct {
  stmt_handle  *stmt_res;
  VALUE        row_number;
  int          arg_count;
  int          funcType;
  VALUE        *error;
  VALUE 	   return_value;
} ifx_db_fetch_helper_args;

void ruby_init_ifx_db();
...
#define RUBY_FE(function) \
  rb_define_singleton_method(mDB, #function, ifx_db_##function, -1);
#define RUBY_FALIAS(alias, function) \
  rb_define_singleton_method(mDB, #alias, ifx_db_##function, -1);
  ...
  static VALUE _ruby_ifx_db_export_str_to_utf16(VALUE string){
  ...
  static VALUE _ruby_ifx_db_export_str_to_ascii(VALUE string){
    rb_encoding *ascii_enc = rb_enc_find("ASCII-8BIT");
    return rb_str_export_to_enc(string,ascii_enc);
  }

  static VALUE _ruby_ifx_db_export_str_to_utf8(VALUE string){
    rb_encoding *utf8_enc = rb_enc_find("UTF-8");
    return rb_str_export_to_enc(string,utf8_enc);
  }

  static VALUE _ruby_ifx_db_export_char_to_utf16_rstr(const char *str){
    VALUE rbString;
    rb_encoding *utf16_enc;
  ...
  static VALUE _ruby_ifx_db_export_char_to_utf8_rstr(const char *str){
    VALUE rbString;
  ...
  static VALUE _ruby_ifx_db_export_sqlwchar_to_utf16_rstr(SQLWCHAR *str, long len) {
    rb_encoding *utf16_enc;
  ...
  static VALUE _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(SQLWCHAR *str, long len){
    VALUE rbString;

    rbString = _ruby_ifx_db_export_sqlwchar_to_utf16_rstr( str, len ); /*Construct the Ruby string from SQLWCHAR(which is in utf16)*/
	
    return _ruby_ifx_db_export_str_to_utf8(rbString); /*Convert the string to utf8 encoding*/
  }

  static VALUE _ruby_ifx_db_export_sqlchar_to_utf8_rstr(SQLCHAR *str, long len) {
    rb_encoding *utf8_enc;
...
/*  Every user visible function must have an entry in Init_ifx_db
*/
void Init_ifx_db(void) {
	
  mDB = rb_define_module("IBM_DB");

  rb_define_module_function(mDB, "connect", ifx_db_connect, -1);
  rb_define_module_function(mDB, "createDB", ifx_db_createDB, -1);
  rb_define_module_function(mDB, "dropDB", ifx_db_dropDB, -1);
  rb_define_module_function(mDB, "createDBNX", ifx_db_createDBNX, -1);
  rb_define_module_function(mDB, "commit", ifx_db_commit, -1);
  rb_define_module_function(mDB, "pconnect", ifx_db_pconnect, -1);
  rb_define_module_function(mDB, "autocommit", ifx_db_autocommit, -1);
  rb_define_module_function(mDB, "bind_param", ifx_db_bind_param, -1);
  rb_define_module_function(mDB, "close", ifx_db_close, -1);
  rb_define_module_function(mDB, "column_privileges", ifx_db_column_privileges, -1);
  rb_define_module_function(mDB, "columnprivileges", ifx_db_column_privileges, -1);
  rb_define_module_function(mDB, "columns", ifx_db_columns, -1);
  rb_define_module_function(mDB, "foreign_keys", ifx_db_foreign_keys, -1);
  rb_define_module_function(mDB, "foreignkeys", ifx_db_foreign_keys, -1);
  rb_define_module_function(mDB, "primary_keys", ifx_db_primary_keys, -1);
  rb_define_module_function(mDB, "primarykeys", ifx_db_primary_keys, -1);
  rb_define_module_function(mDB, "procedure_columns", ifx_db_procedure_columns, -1);
  rb_define_module_function(mDB, "procedurecolumns", ifx_db_procedure_columns, -1);
  rb_define_module_function(mDB, "procedures", ifx_db_procedures, -1);
  rb_define_module_function(mDB, "special_columns", ifx_db_special_columns, -1);
  rb_define_module_function(mDB, "specialcolumns", ifx_db_special_columns, -1);
  rb_define_module_function(mDB, "statistics", ifx_db_statistics, -1);
  rb_define_module_function(mDB, "table_privileges", ifx_db_table_privileges, -1);
  rb_define_module_function(mDB, "tableprivileges", ifx_db_table_privileges, -1);
  rb_define_module_function(mDB, "tables", ifx_db_tables, -1);
  rb_define_module_function(mDB, "exec", ifx_db_exec, -1);
  rb_define_module_function(mDB, "prepare", ifx_db_prepare, -1);
  rb_define_module_function(mDB, "execute", ifx_db_execute, -1);
  rb_define_module_function(mDB, "stmt_errormsg", ifx_db_stmt_errormsg, -1);
  rb_define_module_function(mDB, "conn_errormsg", ifx_db_conn_errormsg, -1);
  rb_define_module_function(mDB, "conn_error", ifx_db_conn_error, -1);
  rb_define_module_function(mDB, "stmt_error", ifx_db_stmt_error, -1);
  rb_define_module_function(mDB, "getErrormsg", ifx_db_getErrormsg, -1);
  rb_define_module_function(mDB, "getErrorstate", ifx_db_getErrorstate, -1);
  rb_define_module_function(mDB, "next_result", ifx_db_next_result, -1);
  rb_define_module_function(mDB, "num_fields", ifx_db_num_fields, -1);
  rb_define_module_function(mDB, "num_rows", ifx_db_num_rows, -1);
  rb_define_module_function(mDB, "resultCols", ifx_db_result_cols, -1);
  rb_define_module_function(mDB, "field_name", ifx_db_field_name, -1);
  rb_define_module_function(mDB, "field_display_size", ifx_db_field_display_size, -1);
  rb_define_module_function(mDB, "field_num", ifx_db_field_num, -1);
  rb_define_module_function(mDB, "field_precision", ifx_db_field_precision, -1);
  rb_define_module_function(mDB, "field_scale", ifx_db_field_scale, -1);
  rb_define_module_function(mDB, "field_type", ifx_db_field_type, -1);
  rb_define_module_function(mDB, "field_width", ifx_db_field_width, -1);
  rb_define_module_function(mDB, "cursor_type", ifx_db_cursor_type, -1);
  rb_define_module_function(mDB, "rollback", ifx_db_rollback, -1);
  rb_define_module_function(mDB, "free_stmt", ifx_db_free_stmt, -1);
  rb_define_module_function(mDB, "result", ifx_db_result, -1);
  rb_define_module_function(mDB, "fetch_row", ifx_db_fetch_row, -1);
  rb_define_module_function(mDB, "fetch_assoc", ifx_db_fetch_assoc, -1);
  rb_define_module_function(mDB, "fetch_array", ifx_db_fetch_array, -1);
  rb_define_module_function(mDB, "fetch_both", ifx_db_fetch_both, -1);
  rb_define_module_function(mDB, "free_result", ifx_db_free_result, -1);
  rb_define_module_function(mDB, "set_option", ifx_db_set_option, -1);
  rb_define_module_function(mDB, "setoption", ifx_db_set_option, -1);
  rb_define_module_function(mDB, "get_option", ifx_db_get_option, -1);
  rb_define_module_function(mDB, "getoption", ifx_db_get_option, -1);
  rb_define_module_function(mDB, "get_last_serial_value", ifx_db_get_last_serial_value, -1);
  rb_define_module_function(mDB, "fetch_object", ifx_db_fetch_object, -1);
  rb_define_module_function(mDB, "server_info", ifx_db_server_info, -1);
  rb_define_module_function(mDB, "client_info", ifx_db_client_info, -1);
  rb_define_module_function(mDB, "active", ifx_db_active, -1);
...
static void ruby_ifx_db_load_necessary_libs() {
  rb_eval_string("require \'bigdecimal\'");
}

#ifdef _WIN32
static void ruby_ifx_db_check_sqlcreatedb(HINSTANCE cliLib) {	
   FARPROC sqlcreatedb;
   sqlcreatedb =  DLSYM( cliLib, "SQLCreateDbW" );
#else
static void ruby_ifx_db_check_sqlcreatedb(void *cliLib) {
 ...
 */
static void ruby_ifx_db_check_if_cli_func_supported() {
#ifdef _WIN32
...
/* On AIX CLI library is in archive. Hence we will need to specify flags in DLOPEN to load a member of the archive*/
  ...
  ruby_ifx_db_check_sqlcreatedb(cliLib);
  DLCLOSE( cliLib );
}

static void ruby_ifx_db_init_globals(struct _ifx_db_globals *ifx_db_globals)
{
  /* env handle */
  ifx_db_globals->bin_mode = 1;

  memset(ifx_db_globals->__ruby_conn_err_msg, '\0', DB2_MAX_ERR_MSG_LEN);
  memset(ifx_db_globals->__ruby_stmt_err_msg, '\0', DB2_MAX_ERR_MSG_LEN);
  memset(ifx_db_globals->__ruby_conn_err_state, '\0', SQL_SQLSTATE_SIZE + 1);
  memset(ifx_db_globals->__ruby_stmt_err_state, '\0', SQL_SQLSTATE_SIZE + 1);
...
/*  static void _ruby_ifx_db_mark_conn_struct */
static void _ruby_ifx_db_mark_conn_struct(conn_handle *handle)
{
}
/*  */

/*  static void _ruby_ifx_db_free_conn_struct */
static void _ruby_ifx_db_free_conn_struct(conn_handle *handle)
{
  ...
  if ( handle != NULL ) {
    ...
    if ( handle->handle_active ) {
      if( handle->transaction_active == 1 && handle->auto_commit == 0 ) {
        ...
        memset(end_X_args,'\0',sizeof(struct _ifx_db_end_tran_args_struct));
        ...
        _ruby_ifx_db_SQLEndTran( end_X_args );

        ruby_xfree( end_X_args );

      }
	  rc = _ruby_ifx_db_SQLDisconnect_helper( &(handle->hdbc) );
...
/*  static void _ruby_ifx_db_mark_pconn_struct */
static void _ruby_ifx_db_mark_pconn_struct(conn_handle *handle)
{
}
/*  */

/*  static void _ruby_ifx_db_free_pconn_struct */
static void _ruby_ifx_db_free_pconn_struct(conn_handle *handle)
{
  _ruby_ifx_db_free_conn_struct( handle );
}
/*  */

/*  static void _ruby_ifx_db_mark_row_struct */
static void _ruby_ifx_db_mark_row_struct(row_hash_struct *handle)
{
  rb_gc_mark( handle->hash );
}
/*  */

/*  static void _ruby_ifx_db_free_row_struct */
static void _ruby_ifx_db_free_row_struct(row_hash_struct *handle)
{
  if ( handle != NULL ) {
    ruby_xfree( handle );
    handle = NULL;
...
/*  static void _ruby_ifx_db_free_result_struct(stmt_handle* handle)
  */
static void _ruby_ifx_db_free_result_struct(stmt_handle* handle)
{
  int i;
...
/*  static _ruby_ifx_db_mark_stmt_struct */
static void _ruby_ifx_db_mark_stmt_struct(stmt_handle *handle)
{
}
...
/*  static _ruby_ifx_db_free_stmt_handle_and_resources */
static void _ruby_ifx_db_free_stmt_handle_and_resources(stmt_handle *handle)
{
  int rc;
  if ( handle != NULL ) {
    if( !handle->is_freed ) {
      ...
      _ruby_ifx_db_free_result_struct( handle );
...
/*  static _ruby_ifx_db_free_stmt_struct */
static void _ruby_ifx_db_free_stmt_struct(stmt_handle *handle)
{		
  if ( handle != NULL ) {
    _ruby_ifx_db_free_stmt_handle_and_resources( handle );
    ruby_xfree( handle );
    handle = NULL;	
  }  
}
...
VALUE ifx_db_row_object(int argc, VALUE *argv, VALUE self)
{
...
void ruby_init_ifx_db()
{
  ...
  ruby_ifx_db_init_globals(ifx_db_globals);
  ...
  /* Specifies the column names case attribute <b>ATTENTION</b> this number is not currently in CLI but used for ifx_db purpose only */
  ...
  tmp_name = INI_STR("ifx_db.instance_name");
  if ( NULL != tmp_name ) {
    ...
    _ruby_ifx_db_instance_name = instance_name;
  }
#endif
  ...
  rb_define_method(le_row_struct, "method_missing", ifx_db_row_object, -1);
  ...
  ruby_ifx_db_load_necessary_libs();

  ruby_ifx_db_check_if_cli_func_supported();
  
}
/*  */

/*  static void _ruby_ifx_db_init_error_info(stmt_handle *stmt_res)
*/
static void _ruby_ifx_db_init_error_info(stmt_handle *stmt_res)
{
  stmt_res->error_recno_tracker     =  1;
...
/*  static void _ruby_ifx_db_check_sql_errors( void *conn_or_stmt, int resourceType, SQLSMALLINT hType, int rc, 
                                           ...
                                           int API, SQLSMALLINT recno, int release_gil )
{
  ...
  memset(get_DiagRec_args,'\0',sizeof( struct _ifx_db_get_diagRec_struct ) );
  ...
  if( release_gil == 1 ){
    ...
    #ifdef UNICODE_SUPPORT_VERSION      
	  ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_SQLGetDiagRec_helper, get_DiagRec_args,
                                (void *)_ruby_ifx_db_Connection_level_UBF, NULL);
	  return_code  =get_DiagRec_args->return_code; 						
    #else
      return_code  =  _ruby_ifx_db_SQLGetDiagRec_helper( get_DiagRec_args );
    #endif
  ...
  if ( return_code == SQL_SUCCESS) {
    ...
    print_str = _ruby_ifx_db_export_str_to_utf16( rb_sprintf( " SQLCODE=%d", (int)sqlcode ) );
    ...
    switch ( rc ) {
      case SQL_ERROR:
        ...
        /* This call was made from ifx_db_errmsg or ifx_db_error */
...
/*  
   static void _ruby_ifx_db_assign_options( void *handle, int type, long opt_key, VALUE data, VALUE *error )
*/
static VALUE _ruby_ifx_db_assign_options( void *handle, int type, long opt_key, VALUE data, VALUE *error )
{
  int    rc            =  0;
  ...
  if (opt_key == ATTR_CASE) {
    ...
    if (type == SQL_HANDLE_STMT) {
      switch (option_num) {
        ...
        default:
#ifdef UNICODE_SUPPORT_VERSION
          *error = _ruby_ifx_db_export_char_to_utf8_rstr("ATTR_CASE attribute must be one of CASE_LOWER, CASE_UPPER, or CASE_NATURAL");
#else
...
#ifdef UNICODE_SUPPORT_VERSION
      *error = _ruby_ifx_db_export_char_to_utf8_rstr("Connection or statement handle must be passed in");
#else
    ...
    memset(handleAttr_args,'\0',sizeof(struct _ifx_db_set_handle_attr_struct));
    ...
    if (TYPE(data) == T_STRING) {
...
#else
      data_utf16  =  _ruby_ifx_db_export_str_to_utf16(data);
      ...
      rc  =  _ruby_ifx_db_SQLSetStmtAttr_helper( handleAttr_args );

      if ( rc == SQL_ERROR ) {
        _ruby_ifx_db_check_sql_errors( handle, DB_STMT, (SQLHSTMT)((stmt_handle *)handle)->hstmt, 
...
#ifdef UNICODE_SUPPORT_VERSION
          *error = rb_str_concat( _ruby_ifx_db_export_char_to_utf8_rstr("Setting of statement attribute failed: "), 
                     _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(((stmt_handle *)handle)->ruby_stmt_err_msg, 
...
#ifdef UNICODE_SUPPORT_VERSION
          *error = _ruby_ifx_db_export_char_to_utf8_rstr("Setting of statement attribute failed: <error message could not be retrieved>");
#else
    ...
    if (TYPE(data) == T_STRING) {
      ...
      rc = _ruby_ifx_db_SQLSetConnectAttr_helper( handleAttr_args );

      if ( rc == SQL_ERROR ) {
        _ruby_ifx_db_check_sql_errors( handle, DB_CONN, (SQLHDBC)((conn_handle *)handle)->hdbc, 
                  SQL_HANDLE_DBC, rc, 1, NULL, NULL, -1, 1, 0 );
        ruby_xfree( handleAttr_args );
        handleAttr_args = NULL;
        if( handle != NULL && ((conn_handle *)handle)->ruby_error_msg != NULL ) {
#ifdef UNICODE_SUPPORT_VERSION
          *error = rb_str_concat( _ruby_ifx_db_export_char_to_utf8_rstr("Setting of connection attribute failed: "),
                     _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(((conn_handle *)handle)->ruby_error_msg,
...
#ifdef UNICODE_SUPPORT_VERSION
          *error = _ruby_ifx_db_export_char_to_utf8_rstr("Setting of connection attribute failed: <error message could not be retrieved>");
#else
...
/*  static int _ruby_ifx_db_parse_options( VALUE options, int type, void *handle, VALUE *error)
*/
static int _ruby_ifx_db_parse_options ( VALUE options, int type, void *handle, VALUE *error )
{
  int   numOpts = 0, i = 0;
  ...
  if ( !NIL_P(options) ) {
    ...
    for ( i = 0; i < numOpts; i++) {
      ...
      if (NUM2LONG(key) == SQL_ATTR_TRUSTED_CONTEXT_PASSWORD) {
        ...
        ret_val  =  _ruby_ifx_db_assign_options( handle, type, NUM2LONG(key), data, error );
      }
    }
    if ( !NIL_P(tc_pass) ) {
      ret_val  =  _ruby_ifx_db_assign_options( handle, type, SQL_ATTR_TRUSTED_CONTEXT_PASSWORD, tc_pass, error );
    }
  }
...
/* 
    static int _ruby_ifx_db_get_result_set_info(stmt_handle *stmt_res)
...
static int _ruby_ifx_db_get_result_set_info(stmt_handle *stmt_res)
{
  ...
  memset(result_cols_args,'\0',sizeof(struct _ifx_db_row_col_count_struct));

  result_cols_args->stmt_res =  stmt_res;
  result_cols_args->count    =  0;

  rc = _ruby_ifx_db_SQLNumResultCols_helper( result_cols_args );

  if ( rc == SQL_ERROR || result_cols_args->count == 0) {
    _ruby_ifx_db_check_sql_errors( stmt_res, DB_STMT, (SQLHSTMT)stmt_res->hstmt, SQL_HANDLE_STMT, rc, 1, NULL, NULL, -1, 1, 0 );
  ...
  stmt_res->column_info = ALLOC_N(ifx_db_result_set_info, result_cols_args->count);
  memset(stmt_res->column_info, 0, sizeof(ifx_db_result_set_info)*(result_cols_args->count));

  describecolargs = ALLOC( describecol_args );
  memset(describecolargs,'\0',sizeof(struct _ifx_db_describecol_args_struct));
  ...
  for (i = 0 ; i < result_cols_args->count; i++) {
    ...
    rc = _ruby_ifx_db_SQLDescribeCol_helper( describecolargs );

    if ( rc == SQL_ERROR ) {
      _ruby_ifx_db_check_sql_errors( stmt_res, DB_STMT, (SQLHSTMT)stmt_res->hstmt, SQL_HANDLE_STMT, 
...
/*  static int _ruby_ifx_db_bind_column_helper(stmt_handle *stmt_res)
...
static int _ruby_ifx_db_bind_column_helper(stmt_handle *stmt_res)
{
  ...
  memset(bindCol_args,'\0',sizeof(struct _ifx_db_bind_col_struct));
  ...
  for (i=0; i<stmt_res->num_columns; i++) {
    ...
    switch(column_type) {
      ...
      case SQL_WLONGVARCHAR:
        ...
        rc = _ruby_ifx_db_SQLBindCol_helper( bindCol_args );
        if ( rc == SQL_ERROR ) {
          _ruby_ifx_db_check_sql_errors( stmt_res, DB_STMT, (SQLHSTMT)stmt_res->hstmt, SQL_HANDLE_STMT,
                    rc, 1, NULL, NULL, -1, 1, 0 );
        }
...
/*  static VALUE _ruby_ifx_db_connect_helper2( connect_helper_args *data )
*/
static VALUE _ruby_ifx_db_connect_helper2( connect_helper_args *data ) {
  ...
  do {
    ...
    if ( isPersistent ) {
      if ( !NIL_P(entry = rb_hash_aref(persistent_list, data->hKey)) ) {		  
        ...
        memset(get_handleAttr_args,'\0',sizeof(struct _ifx_db_get_handle_attr_struct));
        ...
        rc = _ruby_ifx_db_SQLGetConnectAttr_helper( get_handleAttr_args );
		
        ruby_xfree( get_handleAttr_args );
        get_handleAttr_args = NULL;

        if ( (rc == SQL_SUCCESS) && conn_alive ) {
          _ruby_ifx_db_check_sql_errors( conn_res, DB_CONN, conn_res->hdbc, SQL_HANDLE_DBC, rc, 1, NULL, NULL, -1, 1, 0 );
          reused = 1;
...
#endif /* PASE */
      if (rc != SQL_SUCCESS) {
        _ruby_ifx_db_check_sql_errors( conn_res, DB_CONN, conn_res->henv, SQL_HANDLE_ENV, rc, 1, NULL, NULL, -1, 1, 0 );
        ruby_xfree( handleAttr_args );
        handleAttr_args = NULL;
        if( conn_res != NULL && conn_res->ruby_error_msg != NULL ) {
#ifdef UNICODE_SUPPORT_VERSION
          *error = rb_str_concat( _ruby_ifx_db_export_char_to_utf8_rstr("Allocation of environment handle failed: "),
                     _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(conn_res->ruby_error_msg, conn_res->ruby_error_msg_len )
                   );
#else
...
#ifdef UNICODE_SUPPORT_VERSION
          *error = _ruby_ifx_db_export_char_to_utf8_rstr("Allocation of environment handle failed: <error message could not be retrieved>");
#else
...
#endif /* PASE */
      rc = _ruby_ifx_db_SQLSetEnvAttr_helper( handleAttr_args );

      if (rc != SQL_SUCCESS) {
        _ruby_ifx_db_check_sql_errors( conn_res, DB_CONN, conn_res->henv, SQL_HANDLE_ENV, rc, 1, NULL, NULL, -1, 1, 0 );
        if( conn_res != NULL && conn_res->ruby_error_msg != NULL ) {
#ifdef UNICODE_SUPPORT_VERSION
          *error = rb_str_concat( _ruby_ifx_db_export_char_to_utf8_rstr("Setting of Environemnt Attribute during connection failed: "),
                     _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(conn_res->ruby_error_msg, conn_res->ruby_error_msg_len )
                   );
#else
...
#ifdef UNICODE_SUPPORT_VERSION
          *error = _ruby_ifx_db_export_char_to_utf8_rstr("Setting of Environemnt Attribute during connection failed: <error message could not be retrieved>");
#else
        ...
        _ruby_ifx_db_free_conn_struct( conn_res );
        return Qnil;
        break;
      }
    }
    if (! reused) {
      ...
      if (rc != SQL_SUCCESS) {
        ...
        if( conn_res != NULL && conn_res->ruby_error_msg != NULL ) {
#ifdef UNICODE_SUPPORT_VERSION
          *error = rb_str_concat( _ruby_ifx_db_export_char_to_utf8_rstr("Allocation of connection handle failed: "),
                     _ruby_ifx_db_export_sqlwchar_to_utf8_rstr( conn_res->ruby_error_msg, conn_res->ruby_error_msg_len) );
#else
...
#ifdef UNICODE_SUPPORT_VERSION
          *error = _ruby_ifx_db_export_char_to_utf8_rstr("Allocation of connection handle failed: <error message could not be retrieved>");
#else
    ...
    /* Set Options */
    if ( !NIL_P(*options) ) {
      rc = _ruby_ifx_db_parse_options( *options, SQL_HANDLE_DBC, conn_res, error );
      if (rc != SQL_SUCCESS) {
        ruby_xfree( handleAttr_args );
        handleAttr_args = NULL;
    ...
    if (! reused) {
      ...
      rc = _ruby_ifx_db_SQLConnect_helper( conn_args );
      if ( rc == SQL_ERROR ) {
        _ruby_ifx_db_check_sql_errors( conn_res, DB_CONN, conn_res->hdbc, SQL_HANDLE_DBC, rc, 1, NULL, NULL, -1, 1, 0 );
        ...
        if( conn_res != NULL && conn_res->ruby_error_msg != NULL ) {
#ifdef UNICODE_SUPPORT_VERSION
          *error = rb_str_concat( _ruby_ifx_db_export_char_to_utf8_rstr("Connection failed: "),
                     _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(conn_res->ruby_error_msg, conn_res->ruby_error_msg_len));
#else
...
#ifdef UNICODE_SUPPORT_VERSION
          *error = _ruby_ifx_db_export_char_to_utf8_rstr("Connection failed: <error message could not be retrieved>");
#else	
      ...
      if ( rc == SQL_ERROR ) {
        _ruby_ifx_db_check_sql_errors( conn_res, DB_CONN, conn_res->hdbc, SQL_HANDLE_DBC, rc, 1, NULL, NULL, -1, 1, 0 );
		rc = _ruby_ifx_db_SQLDisconnect_helper( &(conn_res->hdbc)  );        
        SQLFreeHandle( SQL_HANDLE_DBC, conn_res->hdbc );
        SQLFreeHandle( SQL_HANDLE_ENV, conn_res->henv );		
        if( conn_res != NULL && conn_res->ruby_error_msg != NULL ) {			
#ifdef UNICODE_SUPPORT_VERSION
          *error = rb_str_concat( _ruby_ifx_db_export_char_to_utf8_rstr("Failed to retrieve autocommit status during connection: "),
                     _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(conn_res->ruby_error_msg, conn_res->ruby_error_msg_len));
#else
          *error = rb_str_cat2(rb_str_new2("Connection failed: "),conn_res->ruby_error_msg);
#endif
...
#ifdef UNICODE_SUPPORT_VERSION
          *error = _ruby_ifx_db_export_char_to_utf8_rstr("Failed to retrieve autocommit status during connection: <error message could not be retrieved>");
#else
          *error = rb_str_new2("Connection failed: <error message could not be retrieved>");
#endif
        }
        ...
        /**
                    ...
                    * */  
       _ruby_ifx_db_set_decfloat_rounding_mode_client( handleAttr_args, conn_res );
      ...
      iserver   =  _ruby_ifx_db_export_char_to_utf16_rstr("AS");
      idsserver =  _ruby_ifx_db_export_char_to_utf16_rstr("IDS");
#else
      server = ALLOC_N(SQLCHAR, 2048);
      memset(server, 0, sizeof(server));
#endif
      getInfo_args = ALLOC( get_info_args );
      memset(getInfo_args,'\0',sizeof(struct _ifx_db_get_info_struct));
...
#endif
      rc = _ruby_ifx_db_SQLGetInfo_helper( getInfo_args );
...
#else
      server_utf16 = _ruby_ifx_db_export_sqlwchar_to_utf16_rstr(server, out_length);
  ...
  if (data->hKey != Qnil) 
  {
	...
	if (! reused && rc == SQL_SUCCESS) 
	{
      ...
      entry = Data_Wrap_Struct(le_pconn_struct,
                _ruby_ifx_db_mark_pconn_struct, _ruby_ifx_db_free_pconn_struct,				
                conn_res);
	  //data->entry = entry;
...
_ruby_ifx_db_mark_conn_struct, _ruby_ifx_db_free_conn_struct,
conn_res);
		*/
  }
...
/*  static int _ruby_ifx_db_connect_helper( argc, argv, isPersistent )
*/
static VALUE _ruby_ifx_db_connect_helper( int argc, VALUE *argv, int isPersistent )
{
  ...
  memset(conn_args,'\0',sizeof(struct _ifx_db_connect_args_struct));
...
#else
  r_db_utf16               =  _ruby_ifx_db_export_str_to_utf16( r_db );
  r_db_ascii               =  _ruby_ifx_db_export_str_to_ascii( r_db );
  r_uid_utf16              =  _ruby_ifx_db_export_str_to_utf16( r_uid );
  r_passwd_utf16           =  _ruby_ifx_db_export_str_to_utf16( r_passwd );
  ...
  memset(helper_args,'\0',sizeof(struct _ifx_db_connect_helper_args_struct));
  ...
  /*If making a persistent connection calculate the hash key to cache the connection in persistence list*/
    ...
    helper_args->hKey = rb_str_concat(rb_str_new2("__ifx_db_"),helper_args->hKey);
#else
    ...
    helper_args->hKey = rb_str_concat(_ruby_ifx_db_export_char_to_utf16_rstr("__ifx_db_"),helper_args->hKey);
#endif
  } else {
  ...
  /* Call the function where the actual logic is being run*/
	...
	ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_connect_helper2, helper_args, (void *)_ruby_ifx_db_Connection_level_UBF, NULL);	
	...
	else
	{
		...
		{
			return_value = Data_Wrap_Struct(le_conn_struct, _ruby_ifx_db_mark_conn_struct, _ruby_ifx_db_free_conn_struct, conn_res);
		}		
	}			 
  #else
    return_value = _ruby_ifx_db_connect_helper2( helper_args );
  #endif
...
*/
static int _ruby_ifx_db_set_decfloat_rounding_mode_client_helper(_rounding_mode *rnd_mode, conn_handle *conn_res ) {
  int rc = 0;
...
#else
  VALUE stmt  =  Qnil;
        stmt  =  _ruby_ifx_db_export_char_to_utf16_rstr("values current decfloat rounding mode");
#endif

  exec_direct_args = ALLOC( exec_cum_prepare_args );
  memset(exec_direct_args,'\0',sizeof(struct _ifx_db_exec_direct_args_struct));
  ...
  rc = _ruby_ifx_db_SQLExecDirect_helper( exec_direct_args );
  ...
  if ( rc == SQL_ERROR ) {
    ...
    _ruby_ifx_db_check_sql_errors( conn_res , DB_CONN, (SQLHSTMT) rnd_mode->stmt_res->hstmt,
             SQL_HANDLE_STMT, rc, 1, NULL, NULL, -1, 1, 0 );
  } else {
    ...
    if ( rc == SQL_ERROR ) {
      ...
      memset(fetch_args,'\0',sizeof(struct _ifx_db_fetch_data_struct));

      fetch_args->stmt_res = rnd_mode->stmt_res;

      rc  = _ruby_ifx_db_SQLFetch_helper( fetch_args );
 ...
 * */
static int _ruby_ifx_db_set_decfloat_rounding_mode_client( set_handle_attr_args *data, conn_handle *conn_res )
{
    int rc = 0;
    ...
    if (rc == SQL_ERROR) {
      _ruby_ifx_db_check_sql_errors( conn_res, DB_CONN, hdbc, SQL_HANDLE_DBC, rc, 1,
                NULL, NULL, -1, 1, 0 );
      ruby_xfree( rnd_mode );
      ruby_xfree( stmt_res );
      rnd_mode = NULL;
    ...
    _ruby_ifx_db_set_decfloat_rounding_mode_client_helper(rnd_mode, conn_res);
	
    _ruby_ifx_db_free_stmt_struct( rnd_mode->stmt_res );
    ...
    rc  =  _ruby_ifx_db_SQLSetConnectAttr_helper( data );
 ...
 * 
 */
VALUE ifx_db_connect(int argc, VALUE *argv, VALUE self)
{
	
  _ruby_ifx_db_clear_conn_err_cache();

  return _ruby_ifx_db_connect_helper( argc, argv, 0 );
}
/*  */
 ...
 */
VALUE ifx_db_pconnect(int argc, VALUE *argv, VALUE self)
{
  _ruby_ifx_db_clear_conn_err_cache();
  return _ruby_ifx_db_connect_helper( argc, argv, 1);
  
}
/*
 * CreateDB helper
 */
VALUE ruby_ifx_db_createDb_helper(VALUE connection, VALUE dbName, VALUE codeSet, VALUE mode, int createNX) {
  ...
  if (!NIL_P(connection)) {
	...
	if (!NIL_P(dbName)) {
	  ...
	  memset(create_db_args,'\0',sizeof(struct _ifx_db_create_drop_db_args_struct));

#ifdef UNICODE_SUPPORT_VERSION
      dbName_utf16               =  _ruby_ifx_db_export_str_to_utf16(dbName);
	  ...
	  if(!NIL_P(codeSet)){
        codeSet_utf16            =  _ruby_ifx_db_export_str_to_utf16(codeSet); 
	  ...
	  if(!NIL_P(mode)) {
	    mode_utf16               =  _ruby_ifx_db_export_str_to_utf16(mode);
...
#ifdef UNICODE_SUPPORT_VERSION        
		ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_SQLCreateDB_helper, create_db_args,
                       (void *)_ruby_ifx_db_Connection_level_UBF, NULL );
		rc = create_db_args->rc;
#else
        rc = _ruby_ifx_db_SQLCreateDB_helper( create_db_args );
#endif
 ...
 */
VALUE ifx_db_createDB(int argc, VALUE *argv, VALUE self)
{
  ...
  return ruby_ifx_db_createDb_helper(connection, dbName, codeSet, mode, 0);  
}
/*
 *  DropDb helper
 */
VALUE ruby_ifx_db_dropDb_helper(VALUE connection, VALUE dbName) {
#ifdef UNICODE_SUPPORT_VERSION
  VALUE dbName_utf16    = Qnil;
#endif
  ...
  if (!NIL_P(connection)) {
	...
	if (!NIL_P(dbName)) {
	  ...
	  memset(drop_db_args,'\0',sizeof(struct _ifx_db_create_drop_db_args_struct));
...
#ifdef UNICODE_SUPPORT_VERSION        
		ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_SQLDropDB_helper, drop_db_args,
                       (void *)_ruby_ifx_db_Connection_level_UBF, NULL );
		rc = drop_db_args->rc;
#else
        rc = _ruby_ifx_db_SQLDropDB_helper( drop_db_args );
#endif
 ...
 * Returns TRUE on success or FALSE on failure. 
 */
VALUE ifx_db_dropDB(int argc, VALUE *argv, VALUE self)
{
  VALUE connection      = Qnil;
  VALUE dbName          = Qnil;
  ...
  return ruby_ifx_db_dropDb_helper(connection, dbName);
}
/*  */
/*
 ...
 * Returns TRUE on success or FALSE on failure. 
 */
/*VALUE ifx_db_recreateDB(int argc, VALUE *argv, VALUE self)
{
  VALUE connection   = Qnil;
  VALUE dbName       = Qnil;
  VALUE codeSet      = Qnil;
  ...
  return_value = ruby_ifx_db_dropDb_helper(connection, dbName);

  if(return_value == Qfalse) {
    return Qfalse;
  }

  return ruby_ifx_db_createDb_helper(connection, dbName, codeSet, mode);  
}*/
/*  */
/*
 ...
 * Returns TRUE on success or FALSE on failure. 
 */
VALUE ifx_db_createDBNX(int argc, VALUE *argv, VALUE self)
{
  VALUE connection   = Qnil;
  VALUE dbName       = Qnil;
  VALUE codeSet      = Qnil;
  ...
  return ruby_ifx_db_createDb_helper(connection, dbName, codeSet, mode, 1);  
}
/*  */
 ...
 * Returns TRUE on success or FALSE on failure. 
 */
VALUE ifx_db_autocommit(int argc, VALUE *argv, VALUE self)
{
  VALUE value;
  VALUE connection = Qnil;
  VALUE ret_val    = Qnil;
  ...
  if (!NIL_P(connection)) {
    ...
    if (argc == 2) {
      ...
      if(autocommit != (conn_res->auto_commit)) {
        ...
        rc = _ruby_ifx_db_SQLSetConnectAttr_helper( handleAttr_args );
        if ( rc == SQL_ERROR ) {
          _ruby_ifx_db_check_sql_errors( conn_res, DB_CONN, conn_res->hdbc, SQL_HANDLE_DBC, rc, 1, NULL, NULL, -1, 1, 1 );
        } else {
...
/*  static void _ruby_ifx_db_add_param_cache( stmt_handle *stmt_res, int param_no, char *varname, int param_type, int size, SQLSMALLINT data_type, SQLSMALLINT precision, SQLSMALLINT scale, SQLSMALLINT nullable )
*/
static void _ruby_ifx_db_add_param_cache( stmt_handle *stmt_res, int param_no, char *varname, int varname_len, int param_type, int size, SQLSMALLINT data_type, SQLUINTEGER precision, SQLSMALLINT scale, SQLSMALLINT nullable )
{
...
/*
  VALUE ifx_db_bind_param_helper(char * varname, long varname_len ,long param_type, long data_type, long precision, 
                                 long scale, long size, stmt_handle *stmt_res, describeparam_args *data)
*/
VALUE ifx_db_bind_param_helper(int argc, char * varname, long varname_len ,long param_type, long data_type, 
  ...
  switch (argc) {
    ...
    case 3:
      param_type = SQL_PARAM_INPUT;
		...
		ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_SQLDescribeParam_helper, data,
                         (void *)_ruby_ifx_db_Statement_level_UBF, stmt_res);
		rc = data->rc;
      #else
        rc  =  _ruby_ifx_db_SQLDescribeParam_helper( data );
      #endif

      if ( rc == SQL_ERROR ) {
        _ruby_ifx_db_check_sql_errors( stmt_res, DB_STMT, stmt_res->hstmt, SQL_HANDLE_STMT, rc, 1, NULL, NULL, -1, 1, 1 );
        return_value = Qfalse;
        break;
      }

      /* Add to cache */
      _ruby_ifx_db_add_param_cache( stmt_res, data->param_no, varname, varname_len, param_type, size, 
  ...
  if( rc == SQL_ERROR ) {
         ...
         */
        err_str = RSTRING_PTR(
                    _ruby_ifx_db_export_sqlwchar_to_utf8_rstr( stmt_res->ruby_stmt_err_msg, 
 ...
 * Returns TRUE on success or FALSE on failure. 
 */
VALUE ifx_db_bind_param(int argc, VALUE *argv, VALUE self)
{
  char *varname   = NULL;
  long varname_len;
  ...
  if (!NIL_P(stmt)) {
    ...
    memset(desc_param_args,'\0',sizeof(struct _ifx_db_describeparam_args_struct));
    ...
    return_value = ifx_db_bind_param_helper(argc,varname,varname_len,param_type,data_type,precision,
 ...
 * Returns TRUE on success or FALSE on failure. 
 */
VALUE ifx_db_close(int argc, VALUE *argv, VALUE self)
{
  VALUE connection = Qnil;
  conn_handle *conn_res;
  int rc;
  ...
  if (!NIL_P(connection)) {
    ...
    if ( conn_res->handle_active && !conn_res->flag_pconnect ) {
      ...
      if (conn_res->auto_commit == 0) {
        ...
        #ifdef UNICODE_SUPPORT_VERSION          
		  ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_SQLEndTran, end_X_args,
                         (void *)_ruby_ifx_db_Connection_level_UBF, NULL);
		  rc = end_X_args->rc;
        #else
          rc = _ruby_ifx_db_SQLEndTran( end_X_args );
        #endif
      ...
      #ifdef UNICODE_SUPPORT_VERSION
	    rc = ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_SQLDisconnect_helper, &(conn_res->hdbc),
                       (void *)_ruby_ifx_db_Connection_level_UBF, NULL);		
      #else
		rc = _ruby_ifx_db_SQLDisconnect_helper( &(conn_res->hdbc) );        
      #endif
 ...
 */
VALUE ifx_db_column_privileges(int argc, VALUE *argv, VALUE self)
{
  ...
  memset(col_privileges_args,'\0',sizeof(struct _ifx_db_metadata_args_struct));
    ...
    if ( !conn_res || !conn_res->handle_active ) {
      ...
      return_value = Qfalse;
        ...
        if (rc == SQL_ERROR) {
...
#ifdef UNICODE_SUPPORT_VERSION
            r_qualifier_utf16                     =  _ruby_ifx_db_export_str_to_utf16( r_qualifier );
          ...
          if (!NIL_P(r_owner)) {
#ifdef UNICODE_SUPPORT_VERSION
            r_owner_utf16                         =   _ruby_ifx_db_export_str_to_utf16( r_owner  );
...
#ifdef UNICODE_SUPPORT_VERSION
            r_table_name_utf16                    =   _ruby_ifx_db_export_str_to_utf16( r_table_name );
...
#ifdef UNICODE_SUPPORT_VERSION
            r_column_name_utf16                   =   _ruby_ifx_db_export_str_to_utf16( r_column_name );
          ...
          #ifdef UNICODE_SUPPORT_VERSION            
			ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_SQLColumnPrivileges_helper, col_privileges_args,
                           (void *)_ruby_ifx_db_Statement_level_UBF, stmt_res );
			rc = col_privileges_args->rc;
          #else
            rc = _ruby_ifx_db_SQLColumnPrivileges_helper( col_privileges_args );
          #endif

          if (rc == SQL_ERROR ) {

            _ruby_ifx_db_check_sql_errors( conn_res, DB_CONN, (SQLHSTMT)stmt_res->hstmt, SQL_HANDLE_STMT, rc, 1, 
                      NULL, NULL, -1, 1, 1 );
            _ruby_ifx_db_free_stmt_struct( stmt_res );
            stmt_res = NULL;

            return_value = Qfalse;
          } else {
            return_value = Data_Wrap_Struct(le_stmt_struct,
            _ruby_ifx_db_mark_stmt_struct, _ruby_ifx_db_free_stmt_struct,
                stmt_res);
 ...
 * Returns FALSE in case of failure.
 */
VALUE ifx_db_columns(int argc, VALUE *argv, VALUE self)
{
  VALUE r_qualifier         =  Qnil;
  VALUE r_owner             =  Qnil;
  VALUE r_table_name        =  Qnil;
  ...
  memset(col_metadata_args,'\0',sizeof(struct _ifx_db_metadata_args_struct));
  ...
  if (!NIL_P(connection)) {
    ...
    if ( !conn_res || !conn_res->handle_active ) {
      ...
      if (rc == SQL_ERROR) {
        ...
        #ifdef UNICODE_SUPPORT_VERSION          
		  ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_SQLColumns_helper, col_metadata_args,
                         (void *)_ruby_ifx_db_Statement_level_UBF, stmt_res );
		  rc = col_metadata_args->rc;
        #else
          rc = _ruby_ifx_db_SQLColumns_helper( col_metadata_args );
        #endif
 ...
 * Returns FALSE in case of failure.
 */
VALUE ifx_db_foreign_keys(int argc, VALUE *argv, VALUE self)
{
  ...
  if (connection) {
    ...
    if (!conn_res->handle_active) {
      ...
      if (rc == SQL_ERROR) {
        ...
        #ifdef UNICODE_SUPPORT_VERSION          
		  ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_SQLForeignKeys_helper, col_metadata_args,
                         (void *)_ruby_ifx_db_Statement_level_UBF, stmt_res );
		  rc = col_metadata_args->rc;
        #else
          rc = _ruby_ifx_db_SQLForeignKeys_helper( col_metadata_args );
        #endif

        if (rc == SQL_ERROR ) {

          _ruby_ifx_db_check_sql_errors( conn_res, DB_CONN, stmt_res->hstmt, SQL_HANDLE_STMT, rc, 1, 
                    NULL, NULL, -1, 1, 1 );
          _ruby_ifx_db_free_stmt_struct( stmt_res );
          stmt_res = NULL;
 ...
 * Returns FALSE in case of failure.
 */
VALUE ifx_db_primary_keys(int argc, VALUE *argv, VALUE self)
{
  VALUE r_qualifier         =  Qnil;
  VALUE r_owner             =  Qnil;
  VALUE r_table_name        =  Qnil;
  ...
  if (connection) {
    ...
    if (!conn_res->handle_active) {
      ...
      if (rc == SQL_ERROR) {
        ...
        #ifdef UNICODE_SUPPORT_VERSION          
		  ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_SQLPrimaryKeys_helper, col_metadata_args,
                         (void *)_ruby_ifx_db_Statement_level_UBF, stmt_res );
		  rc = col_metadata_args->rc;
        #else
          rc = _ruby_ifx_db_SQLPrimaryKeys_helper( col_metadata_args );
        #endif
 ...
 * Returns FALSE in case of failure.
 */
VALUE ifx_db_procedure_columns(int argc, VALUE *argv, VALUE self)
{
  VALUE r_qualifier          =  Qnil;
  VALUE r_owner              =  Qnil;
  ...
  if (!NIL_P(connection)) {
    ...
    if (!conn_res || !conn_res->handle_active) {
      ...
      if (rc == SQL_ERROR) {
...
#ifdef UNICODE_SUPPORT_VERSION
          r_proc_name_utf16                      =  _ruby_ifx_db_export_str_to_utf16( r_proc_name );
        ...
        #ifdef UNICODE_SUPPORT_VERSION          
		  ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_SQLProcedureColumns_helper, col_metadata_args,
                         (void *)_ruby_ifx_db_Statement_level_UBF, stmt_res);
		  rc = col_metadata_args->rc;
        #else
          rc = _ruby_ifx_db_SQLProcedureColumns_helper( col_metadata_args );
        #endif
 ...
 * Returns FALSE in case of failure.
 */
VALUE ifx_db_procedures(int argc, VALUE *argv, VALUE self)
{
  VALUE r_qualifier  =  Qnil;
  VALUE r_owner      =  Qnil;
  VALUE r_proc_name  =  Qnil;
  ...
  memset(proc_metadata_args,'\0',sizeof(struct _ifx_db_metadata_args_struct));
  ...
  if (!NIL_P(connection)) {
    ...
    if (!conn_res || !conn_res->handle_active) {
      ...
      if (rc == SQL_ERROR) {
        ...
        #ifdef UNICODE_SUPPORT_VERSION          
		  ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_SQLProcedures_helper, proc_metadata_args,
                         (void *)_ruby_ifx_db_Statement_level_UBF, stmt_res);
		  rc = proc_metadata_args->rc;
        #else
          rc = _ruby_ifx_db_SQLProcedures_helper( proc_metadata_args );
        #endif
 ...
 * Returns FALSE in case of failure.
 */
VALUE ifx_db_special_columns(int argc, VALUE *argv, VALUE self)
{
  VALUE r_qualifier         =  Qnil;
  VALUE r_owner             =  Qnil;
  VALUE r_table_name        =  Qnil;
  ...
  if (!NIL_P(connection)) {
    ...
    if (!conn_res || !conn_res->handle_active) {
      ...
      if (rc == SQL_ERROR) {
        ...
        #ifdef UNICODE_SUPPORT_VERSION          
		  ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_SQLSpecialColumns_helper, col_metadata_args,
                         (void *)_ruby_ifx_db_Statement_level_UBF, stmt_res);
		  rc = col_metadata_args->rc;				 
        #else
          rc = _ruby_ifx_db_SQLSpecialColumns_helper( col_metadata_args );
        #endif
 ...
 * Returns FALSE in case of failure.
 */
VALUE ifx_db_statistics(int argc, VALUE *argv, VALUE self)
{
  ...
  if (!NIL_P(connection)) {
    ...
    if (!conn_res || !conn_res->handle_active) {
      ...
      if (rc == SQL_ERROR) {
        ...
        #ifdef UNICODE_SUPPORT_VERSION          
		  ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_SQLStatistics_helper, col_metadata_args,
                         (void *)_ruby_ifx_db_Statement_level_UBF, stmt_res );
		   rc = col_metadata_args->rc;
        #else
          rc = _ruby_ifx_db_SQLStatistics_helper( col_metadata_args );
        #endif
 ...
 * Returns FALSE in case of failure.
 */
VALUE ifx_db_table_privileges(int argc, VALUE *argv, VALUE self)
{
  VALUE r_qualifier         =  Qnil;
  VALUE r_owner             =  Qnil;
  VALUE r_table_name        =  Qnil;
  ...
  memset(table_privileges_args,'\0',sizeof(struct _ifx_db_metadata_args_struct));
  ...
  if (!NIL_P(connection)) {
    ...
    if (!conn_res || !conn_res->handle_active) {
      ...
      if (rc == SQL_ERROR) {
        ...
        #ifdef UNICODE_SUPPORT_VERSION          
		  ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_SQLTablePrivileges_helper, table_privileges_args,
                         (void *)_ruby_ifx_db_Statement_level_UBF, stmt_res );
		  rc = table_privileges_args->rc;
        #else
          rc = _ruby_ifx_db_SQLTablePrivileges_helper( table_privileges_args );
        #endif
 ...
 * Returns FALSE in case of failure.
 */
VALUE ifx_db_tables(int argc, VALUE *argv, VALUE self)
{
  VALUE r_qualifier         =  Qnil;
  VALUE r_owner             =  Qnil;
  VALUE r_table_name        =  Qnil;
  ...
  memset(table_metadata_args,'\0',sizeof(struct _ifx_db_metadata_args_struct));
  ...
  if (!NIL_P(connection)) {
    ...
    if (!conn_res || !conn_res->handle_active) {
      ...
      if (rc == SQL_ERROR) {
...
#ifdef UNICODE_SUPPORT_VERSION
          r_table_type_utf16                     =  _ruby_ifx_db_export_str_to_utf16( r_table_type );
        ...
        #ifdef UNICODE_SUPPORT_VERSION          
		  ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_SQLTables_helper, table_metadata_args,
                         (void *)_ruby_ifx_db_Statement_level_UBF, stmt_res );
		  rc = table_metadata_args->rc;
        #else
          rc = _ruby_ifx_db_SQLTables_helper( table_metadata_args );
        #endif
 ...
 * Returns TRUE on success or FALSE on failure. 
 */
VALUE ifx_db_commit(int argc, VALUE *argv, VALUE self)
{
  VALUE connection = Qnil;
  conn_handle *conn_res;
  int rc;
...
/*  static int _ruby_ifx_db_do_prepare(conn_handle *conn_res, VALUE stmt, stmt_handle *stmt_res, 
                                       VALUE options)
*/
static int _ruby_ifx_db_do_prepare(conn_handle *conn_res, VALUE stmt, stmt_handle *stmt_res, 
                                   VALUE options)
{
  ...
  if ( !NIL_P(stmt) ) {
    ...
    memset(prepare_args,'\0',sizeof(struct _ifx_db_exec_direct_args_struct));
#ifdef UNICODE_SUPPORT_VERSION
    stmt_utf16                     =  _ruby_ifx_db_export_str_to_utf16(stmt);
  ...
  if ( rc < SQL_SUCCESS ) {
    ...
    if ( !NIL_P(options) ) {
      rc = _ruby_ifx_db_parse_options( options, SQL_HANDLE_STMT, stmt_res, &error );
      if ( rc == SQL_ERROR ) {
    ...
    /* Prepare the stmt. The cursor type requested has already been set in _ruby_ifx_db_assign_options */
    #ifdef UNICODE_SUPPORT_VERSION      
	  ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_SQLPrepare_helper, prepare_args,
                     (void *)_ruby_ifx_db_Statement_level_UBF, stmt_res );
	  rc = prepare_args->rc;
    #else
      rc = _ruby_ifx_db_SQLPrepare_helper( prepare_args );
    #endif
 ...
 */
VALUE ifx_db_exec(int argc, VALUE *argv, VALUE self)
{
  VALUE stmt         =  Qnil;
#ifdef UNICODE_SUPPORT_VERSION
  VALUE stmt_utf16   =  Qnil;
  ...
  /* This function basically is a wrap of the _ruby_ifx_db_do_prepare and _ruby_ifx_db_execute_stmt */
  ...
  if (!NIL_P(connection)) {
    ...
    _ruby_ifx_db_clear_stmt_err_cache();
    ...
    if ( rc < SQL_SUCCESS ) {
      ...
      #ifdef UNICODE_SUPPORT_VERSION        
		ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_SQLExecDirect_helper, exec_direct_args,
                       (void *)_ruby_ifx_db_Statement_level_UBF, stmt_res );
      ...
      if ( rc == SQL_ERROR ) {
        ...
        _ruby_ifx_db_check_sql_errors( conn_res, DB_CONN, stmt_res->hstmt, 
 ...
 * Returns TRUE on success or FALSE on failure. 
 */
VALUE ifx_db_free_result(int argc, VALUE *argv, VALUE self)
{
  VALUE stmt   =  Qnil;
  int   rc     =  0;
  ...
  if (!NIL_P(stmt)) {
    ...
    if ( stmt_res->hstmt ) {
      ...
      memset(freeStmt_args, '\0', sizeof( struct _ifx_db_free_stmt_struct ) );

      freeStmt_args->stmt_res  =  stmt_res;
      freeStmt_args->option    =  SQL_CLOSE;

      #ifdef UNICODE_SUPPORT_VERSION        
		ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_SQLFreeStmt_helper, freeStmt_args, 
                       (void *)_ruby_ifx_db_Statement_level_UBF, stmt_res );
		rc = freeStmt_args->rc;
      #else
        rc = _ruby_ifx_db_SQLFreeStmt_helper( freeStmt_args );
      #endif
    ...
    _ruby_ifx_db_free_result_struct( stmt_res );
  } else {
    rb_warn("Supplied parameter is invalid");
    return Qfalse;
  }
 ...
 */
VALUE ifx_db_prepare(int argc, VALUE *argv, VALUE self)
{
  VALUE stmt         =  Qnil;
  VALUE connection   =  Qnil;
  VALUE options      =  Qnil;
  ...
  if (!NIL_P(connection)) {
    ...
    /* returns the stat_handle back to the calling function */
    rc = _ruby_ifx_db_do_prepare( conn_res, stmt, stmt_res, options );
...
#ifdef UNICODE_SUPPORT_VERSION
      err_str = RSTRING_PTR( _ruby_ifx_db_export_sqlwchar_to_utf8_rstr( conn_res->ruby_error_msg,
                                          DB2_MAX_ERR_MSG_LEN * sizeof(SQLWCHAR) )
                           );
...
/*
   static int _ruby_ifx_db_bind_parameter_helper(stmt_handle *stmt_res, param_node *curr, VALUE *bind_data)
...
static int _ruby_ifx_db_bind_parameter_helper(stmt_handle *stmt_res, param_node *curr, VALUE *bind_data) {
  ...
  memset(bindParameter_args,'\0',sizeof(struct _ifx_db_bind_parameter_struct));
  ...
  switch(TYPE(*bind_data)) {
    case T_BIGNUM:
      ...
      rc = _ruby_ifx_db_SQLBindParameter_helper( bindParameter_args );
    ...
    case T_STRING:
     ...
     if( is_binary ){
       ...
       bindData_utf16 = _ruby_ifx_db_export_str_to_utf16( *bind_data );
  ...
  if ( rc == SQL_ERROR && is_known_type == 1 ) {
     _ruby_ifx_db_check_sql_errors( stmt_res, DB_STMT, stmt_res->hstmt, SQL_HANDLE_STMT, rc, 1, NULL, NULL, -1, 1, 0 );
  }
...
/*  static int _ruby_ifx_db_bind_data( stmt_handle *stmt_res, param_node *curr, VALUE *bind_data )
*/
static int _ruby_ifx_db_bind_data( stmt_handle *stmt_res, param_node *curr, VALUE *bind_data)
{
  int rc;
  ...
  if ( curr->param_type == PARAM_FILE) {
    ...
    rc = _ruby_ifx_db_SQLBindFileToParam_helper(stmt_res, curr);
  ...
  rc = _ruby_ifx_db_bind_parameter_helper(stmt_res, curr, bind_data);

  return rc;
}
/*
  static int _ruby_ifx_db_bind_param_list(stmt_handle *stmt_res, VALUE *error)
*/
static int _ruby_ifx_db_bind_param_list(stmt_handle *stmt_res, VALUE *error ) {
  int rc;
  ...
  while (curr != NULL ) {
    ...
    rc  = _ruby_ifx_db_bind_data( stmt_res, curr, &bind_data);

    if ( rc == SQL_ERROR ) {
      if( stmt_res != NULL && stmt_res->ruby_stmt_err_msg != NULL ) {
#ifdef UNICODE_SUPPORT_VERSION
        *error = rb_str_concat( _ruby_ifx_db_export_char_to_utf8_rstr("Binding Error 1: "),
                   _ruby_ifx_db_export_sqlwchar_to_utf8_rstr( stmt_res->ruby_stmt_err_msg,
                                stmt_res->ruby_stmt_err_msg_len )
                 );
#else
...
#ifdef UNICODE_SUPPORT_VERSION
        *error = _ruby_ifx_db_export_char_to_utf8_rstr("Binding Error 1: <error message could not be retrieved>");
#else
...
/*  static int _ruby_ifx_db_execute_helper2(stmt_res, data, int bind_cmp_list, int bind_params, VALUE *error )
  */
static int _ruby_ifx_db_execute_helper2(stmt_handle *stmt_res, VALUE *data, int bind_cmp_list, 
                                        int bind_params, VALUE *error )
{
  ...
  if ( bind_cmp_list ) {
    ...
    return _ruby_ifx_db_bind_param_list( stmt_res, error );
    ...
    if ( data != NULL ) {
      if ( bind_params ) {
        ...
        */
        ...
        rc = _ruby_ifx_db_SQLDescribeParam_helper( desc_param_args );
        ...
        if ( rc == SQL_ERROR ) {
          ...
          if( stmt_res != NULL && stmt_res->ruby_stmt_err_msg != NULL ) {
#ifdef UNICODE_SUPPORT_VERSION
            *error = rb_str_concat( _ruby_ifx_db_export_char_to_utf8_rstr( "Describe Param Failed: " ),
                       _ruby_ifx_db_export_sqlwchar_to_utf8_rstr( stmt_res->ruby_stmt_err_msg,
                                    stmt_res->ruby_stmt_err_msg_len )
                     );
#else
...
#ifdef UNICODE_SUPPORT_VERSION
            *error = _ruby_ifx_db_export_char_to_utf8_rstr("Describe Param Failed: <error message could not be retrieved>");
#else
        ...
        rc = _ruby_ifx_db_bind_data( stmt_res, curr, data );

        if ( rc == SQL_ERROR ) {
          if( stmt_res != NULL && stmt_res->ruby_stmt_err_msg != NULL ) {
#ifdef UNICODE_SUPPORT_VERSION
            *error = rb_str_concat( _ruby_ifx_db_export_char_to_utf8_rstr("Binding Error 2: "),
                       _ruby_ifx_db_export_sqlwchar_to_utf8_rstr( stmt_res->ruby_stmt_err_msg,
                                    stmt_res->ruby_stmt_err_msg_len )
                     );
#else
...
#ifdef UNICODE_SUPPORT_VERSION
            *error = _ruby_ifx_db_export_char_to_utf8_rstr("Binding Error 2: <error message could not be retrieved>");
#else
...
/* 
  static VALUE _ruby_ifx_db_desc_and_bind_param_list(stmt_bind_array *bind_array, VALUE *error)
*/
static VALUE _ruby_ifx_db_desc_and_bind_param_list(stmt_bind_array *bind_array, VALUE *error ) {
  int rc, numOpts, i = 0;
...
#ifdef UNICODE_SUPPORT_VERSION
    *error = _ruby_ifx_db_export_char_to_utf8_rstr("Number of params passed in are less than bound parameters");
#else
  ...
  for ( i = 0; i < numOpts; i++) {
    ...
    */
    ...
    rc  =  _ruby_ifx_db_execute_helper2(bind_array->stmt_res, &data, 0, bind_array->bind_params, error );
...
/*  static int _ruby_ifx_db_execute_helper(stmt_res, data, int bind_cmp_list)
*/
static VALUE _ruby_ifx_db_execute_helper(stmt_bind_array *bind_array) {
  ...
  _ruby_ifx_db_SQLFreeStmt_helper( freeStmt_args );

  ruby_xfree( freeStmt_args );
  freeStmt_args = NULL;

  num_params_args = ALLOC( row_col_count_args );
  memset(num_params_args,'\0',sizeof(struct _ifx_db_row_col_count_struct));

  num_params_args->stmt_res  =  stmt_res;
  num_params_args->count     =  0;

  rc   =  _ruby_ifx_db_SQLNumParams_helper( num_params_args );
  ...
  if ( rc == SQL_ERROR ) {
    ...
    if( stmt_res != NULL && stmt_res->ruby_stmt_err_msg != NULL ) {
#ifdef UNICODE_SUPPORT_VERSION
      *error = rb_str_concat( _ruby_ifx_db_export_char_to_utf8_rstr("Execute Failed due to: "),
                 _ruby_ifx_db_export_sqlwchar_to_utf8_rstr( stmt_res->ruby_stmt_err_msg, 
                              stmt_res->ruby_stmt_err_msg_len )
               );
#else
...
#ifdef UNICODE_SUPPORT_VERSION
      *error = _ruby_ifx_db_export_char_to_utf8_rstr("Execute Failed due to: <error message could not be retrieved>");
#else
  ...
  if ( num != 0 ) {
    ...
    if ( !NIL_P( *(bind_array->parameters_array) ) ) {
...
#ifdef UNICODE_SUPPORT_VERSION
        *error = _ruby_ifx_db_export_char_to_utf8_rstr("Param is not an array");
#else
      ...
      ret_value  =  _ruby_ifx_db_desc_and_bind_param_list(bind_array, error );
...
#ifdef UNICODE_SUPPORT_VERSION
        *error = _ruby_ifx_db_export_char_to_utf8_rstr("Number of params passed are more than bound parameters");
#else
...
#ifdef UNICODE_SUPPORT_VERSION
        *error = _ruby_ifx_db_export_char_to_utf8_rstr("Number of params passed are less than bound parameters");
#else
      ...
      if ( stmt_res->head_cache_list == NULL ) {
#ifdef UNICODE_SUPPORT_VERSION
        *error = _ruby_ifx_db_export_char_to_utf8_rstr("Parameters not bound");
#else
        *error = rb_str_new2("Parameters not bound");
        ...
        rc  =  _ruby_ifx_db_execute_helper2(stmt_res, NULL, 1, 0, error );
  ...
  rc  =  _ruby_ifx_db_SQLExecute_helper( stmt_res );

  if ( rc == SQL_ERROR ) {
    _ruby_ifx_db_check_sql_errors( stmt_res, DB_STMT, stmt_res->hstmt, SQL_HANDLE_STMT, rc, 1, NULL, NULL, -1, 1, 0 );
    if( stmt_res != NULL && stmt_res->ruby_stmt_err_msg != NULL ) {
#ifdef UNICODE_SUPPORT_VERSION
      *error = rb_str_concat( _ruby_ifx_db_export_char_to_utf8_rstr("Statement Execute Failed: "),
                 _ruby_ifx_db_export_sqlwchar_to_utf8_rstr( stmt_res->ruby_stmt_err_msg,
                              stmt_res->ruby_stmt_err_msg_len )
               );
#else
...
#ifdef UNICODE_SUPPORT_VERSION
      *error = _ruby_ifx_db_export_char_to_utf8_rstr("Statement Execute Failed: <error message could not be retrieved>");
#else
  ...
  if ( rc == SQL_NEED_DATA ) {
    ...
    memset(put_param_data_args,'\0',sizeof(struct _ifx_db_param_and_put_data_struct));

    put_param_data_args->stmt_res  =  stmt_res;

    rc = _ruby_ifx_db_SQLParamData_helper( put_param_data_args );
    while ( rc == SQL_NEED_DATA ) {

      /* passing data value for a parameter */
      rc  = _ruby_ifx_db_SQLPutData_helper(put_param_data_args);

      if ( rc == SQL_ERROR ) {
        _ruby_ifx_db_check_sql_errors( stmt_res, DB_STMT, stmt_res->hstmt, SQL_HANDLE_STMT, rc, 1, NULL, NULL, -1, 1, 0 );
        if( stmt_res != NULL && stmt_res->ruby_stmt_err_msg != NULL ) {
#ifdef UNICODE_SUPPORT_VERSION
          *error = rb_str_concat( _ruby_ifx_db_export_char_to_utf8_rstr( "Sending data failed: "),
                     _ruby_ifx_db_export_sqlwchar_to_utf8_rstr( stmt_res->ruby_stmt_err_msg,
                                  stmt_res->ruby_stmt_err_msg_len )
                   );
#else
...
#ifdef UNICODE_SUPPORT_VERSION
          *error = _ruby_ifx_db_export_char_to_utf8_rstr("Sending data failed: <error message could not be retrieved>");
#else
 ...
 * Returns TRUE on success or FALSE on failure.
 */
VALUE ifx_db_execute(int argc, VALUE *argv, VALUE self)
{
  VALUE stmt                   =  Qnil;
  ...
  if (!NIL_P(stmt)) {
    ...
    #ifdef UNICODE_SUPPORT_VERSION      
	  ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_execute_helper, bind_array,
                            (void *)_ruby_ifx_db_Statement_level_UBF, stmt_res );
	  ret_value = bind_array->return_value;
    #else
      ret_value = _ruby_ifx_db_execute_helper( bind_array );
    #endif
              ...
              switch (tmp_curr->data_type) {
                ...
                case SQL_XML:
                    ...
                    if( tmp_curr-> size > 0 && tmp_curr->bind_indicator > tmp_curr->size ){
                      rb_funcall(rb_eval_string(tmp_curr->varname), rb_intern("replace"), 1, _ruby_ifx_db_export_sqlchar_to_utf8_rstr( (SQLCHAR *) tmp_curr->svalue, tmp_curr->size ));
                    } else {
                      rb_funcall(rb_eval_string(tmp_curr->varname), rb_intern("replace"), 1, _ruby_ifx_db_export_sqlchar_to_utf8_rstr( (SQLCHAR *) tmp_curr->svalue, tmp_curr->bind_indicator));
                    }
#else
                ...
                default:
                    ...
                    if( tmp_curr-> size > 0 && tmp_curr->bind_indicator > (tmp_curr->size * sizeof(SQLWCHAR))+ 2 ){
                      rb_funcall(rb_eval_string(tmp_curr->varname), rb_intern("replace"), 1, _ruby_ifx_db_export_sqlwchar_to_utf8_rstr( (SQLWCHAR *) tmp_curr->svalue, (tmp_curr->size * sizeof(SQLWCHAR)) + 2 ));
                    } else {
                      rb_funcall(rb_eval_string(tmp_curr->varname), rb_intern("replace"), 1, _ruby_ifx_db_export_sqlwchar_to_utf8_rstr( (SQLWCHAR *) tmp_curr->svalue, tmp_curr->bind_indicator));
                    }
#else
                  var_assign(tmp_curr->varname, rb_str_new2((char *)tmp_curr->svalue));
#endif
 ...
 * Use getErrormsg
 */
VALUE ifx_db_conn_errormsg(int argc, VALUE *argv, VALUE self)
{
  VALUE         connection      =  Qnil;
  VALUE         ret_val         =  Qnil;
  conn_handle   *conn_res       =  NULL;
  SQLPOINTER    return_str      =  NULL;  /* This variable is used by _ruby_ifx_db_check_sql_errors to return err strings */
  ...
  if (!NIL_P(connection)) {
    ...
    _ruby_ifx_db_check_sql_errors(conn_res, DB_CONN, conn_res->hdbc, SQL_HANDLE_DBC, -1, 0, return_str, &return_str_len, DB_ERRMSG, conn_res->errormsg_recno_tracker, 1 );
...
#ifdef UNICODE_SUPPORT_VERSION
    ret_val  =  _ruby_ifx_db_export_sqlwchar_to_utf8_rstr( (SQLWCHAR *)return_str, return_str_len );
#else
...
#ifdef UNICODE_SUPPORT_VERSION
    return _ruby_ifx_db_export_sqlwchar_to_utf8_rstr( IBM_DB_G(__ruby_conn_err_msg), DB2_MAX_ERR_MSG_LEN * sizeof(SQLWCHAR) );
#else
 ...
 * Use getErrormsg
 */
VALUE ifx_db_stmt_errormsg(int argc, VALUE *argv, VALUE self)
{
  VALUE        stmt            =  Qnil;
  VALUE        ret_val         =  Qnil;
  stmt_handle  *stmt_res       =  NULL;
  ...
  if (!NIL_P(stmt)) {
    ...
    _ruby_ifx_db_check_sql_errors(stmt_res, DB_STMT, stmt_res->hstmt, SQL_HANDLE_STMT, -1, 0, 
...
#ifdef UNICODE_SUPPORT_VERSION
    ret_val  =  _ruby_ifx_db_export_sqlwchar_to_utf8_rstr( return_str, return_str_len );
#else
    ret_val  =  rb_str_new2( return_str );
#endif
    ruby_xfree( return_str );
...
#ifdef UNICODE_SUPPORT_VERSION
    return _ruby_ifx_db_export_sqlwchar_to_utf8_rstr( IBM_DB_G(__ruby_stmt_err_msg), DB2_MAX_ERR_MSG_LEN * sizeof(SQLWCHAR) );
#else
 ...
 * Use getErrorstate
 */
VALUE ifx_db_conn_error(int argc, VALUE *argv, VALUE self)
{
  VALUE         connection      =  Qnil;
  VALUE         ret_val         =  Qnil;
  conn_handle   *conn_res       =  NULL;
  ...
  if (!NIL_P(connection)) {
    ...
    _ruby_ifx_db_check_sql_errors(conn_res, DB_CONN, conn_res->hdbc, SQL_HANDLE_DBC, -1, 0, 
...
#ifdef UNICODE_SUPPORT_VERSION
    ret_val = _ruby_ifx_db_export_sqlwchar_to_utf8_rstr( return_str, SQL_SQLSTATE_SIZE * sizeof(SQLWCHAR) );
#else
    ret_val = rb_str_new2( return_str );
#endif
    ruby_xfree( return_str );
...
#ifdef UNICODE_SUPPORT_VERSION
    return _ruby_ifx_db_export_sqlwchar_to_utf8_rstr( IBM_DB_G(__ruby_conn_err_state),
 ...
 * Use getErrorstate
 */
VALUE ifx_db_stmt_error(int argc, VALUE *argv, VALUE self)
{
  VALUE         stmt            =  Qnil;
  VALUE         ret_val         =  Qnil;
  stmt_handle   *stmt_res       =  NULL;
...
#ifdef UNICODE_SUPPORT_VERSION
    return _ruby_ifx_db_export_sqlwchar_to_utf8_rstr( IBM_DB_G(__ruby_stmt_err_state), 
             SQL_SQLSTATE_SIZE * sizeof(SQLWCHAR) 
           );
#else
 ...
 *
 */
VALUE ifx_db_getErrormsg(int argc, VALUE *argv, VALUE self)
{
  ...
  if (!NIL_P(conn_or_stmt) && !NIL_P(resourceType) ) {
    ...
    if( resType == DB_CONN ) {   /*Resource Type is connection*/
      ...
      if( conn_res->errorType != 1 ) {
        if( conn_res->ruby_error_msg != NULL ) {
#ifdef UNICODE_SUPPORT_VERSION
          return_value  =  _ruby_ifx_db_export_sqlwchar_to_utf8_rstr( conn_res->ruby_error_msg, conn_res->ruby_error_msg_len );
#else
        ...
        if( conn_res->errormsg_recno_tracker == 1 && conn_res->ruby_error_msg != NULL ) {
          ...
          _ruby_ifx_db_check_sql_errors(conn_res, DB_CONN, conn_res->hdbc, SQL_HANDLE_DBC, -1, 0, return_str,
...
#ifdef UNICODE_SUPPORT_VERSION
          return_value = _ruby_ifx_db_export_sqlwchar_to_utf8_rstr( return_str, return_str_len );
#else
...
#ifdef UNICODE_SUPPORT_VERSION
        return_value  =  _ruby_ifx_db_export_sqlwchar_to_utf8_rstr( stmt_res->ruby_stmt_err_msg, stmt_res->ruby_stmt_err_msg_len );
#else
 ...
 *
 */
VALUE ifx_db_getErrorstate(int argc, VALUE *argv, VALUE self)
{
  VALUE        conn_or_stmt    =  Qnil;
  VALUE        resourceType    =  Qnil;
  ...
  if (!NIL_P(conn_or_stmt) && !NIL_P(resourceType) ) {
    ...
    if( resType == DB_CONN ) {   /*Resource Type is connection*/
      ...
      if( conn_res->errorType != 1 ) {
          if( conn_res->ruby_error_state != NULL ) {
#ifdef UNICODE_SUPPORT_VERSION
            return_value  =  _ruby_ifx_db_export_sqlwchar_to_utf8_rstr( conn_res->ruby_error_state,
                                SQL_SQLSTATE_SIZE * sizeof(SQLWCHAR) 
                             );
#else
...
#ifdef UNICODE_SUPPORT_VERSION
          return_value = _ruby_ifx_db_export_sqlwchar_to_utf8_rstr( return_str, SQL_SQLSTATE_SIZE * sizeof(SQLWCHAR) );
#else
          return_value = rb_str_new2( return_str );
#endif
          ruby_xfree( return_str );
...
#ifdef UNICODE_SUPPORT_VERSION
        return_value  =  _ruby_ifx_db_export_sqlwchar_to_utf8_rstr( stmt_res->ruby_stmt_err_state, 
                            SQL_SQLSTATE_SIZE * sizeof(SQLWCHAR) 
                         );
#else
...
#ifdef UNICODE_SUPPORT_VERSION
        return_value = _ruby_ifx_db_export_sqlwchar_to_utf8_rstr( return_str,
                          SQL_SQLSTATE_SIZE * sizeof(SQLWCHAR)
                       );
#else
        return_value = rb_str_new2( return_str );
 ...
 */
VALUE ifx_db_next_result(int argc, VALUE *argv, VALUE self)
{
  VALUE stmt      = Qnil;
  ...
  if (!NIL_P(stmt)) {
    ...
    if ( rc < SQL_SUCCESS ) {
      ...
      _ruby_ifx_db_check_sql_errors( stmt_res , DB_STMT, stmt_res->hdbc, SQL_HANDLE_DBC, rc, 0, 
      ...
      memset(nextresultparams,'\0',sizeof(struct _ifx_db_next_result_args_struct));

      nextresultparams->stmt_res  =  stmt_res;
      nextresultparams->new_hstmt =  &new_hstmt;

      #ifdef UNICODE_SUPPORT_VERSION        
		ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_SQLNextResult_helper, nextresultparams,
                       (void *)_ruby_ifx_db_Statement_level_UBF, stmt_res );
		rc = nextresultparams->rc;
      #else
        rc = _ruby_ifx_db_SQLNextResult_helper( nextresultparams );
      #endif
 ...
 */
VALUE ifx_db_num_fields(int argc, VALUE *argv, VALUE self)
{
  VALUE stmt    = Qnil;
  VALUE ret_val = Qnil;
  stmt_handle *stmt_res;
  ...
  if (!NIL_P(stmt)) {
    ...
    #ifdef UNICODE_SUPPORT_VERSION      
	  ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_SQLNumResultCols_helper, result_cols_args,
                     (void *)_ruby_ifx_db_Statement_level_UBF, stmt_res );
    ...
    if ( rc == SQL_ERROR ) {
      _ruby_ifx_db_check_sql_errors( stmt_res, DB_STMT, stmt_res->hstmt, SQL_HANDLE_STMT, rc, 1, NULL, NULL, -1, 1, 1 );
#ifdef UNICODE_SUPPORT_VERSION
      err_str = RSTRING_PTR( _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(stmt_res->ruby_stmt_err_msg,
                                          stmt_res->ruby_stmt_err_msg_len )
                           );
 ...
 * Returns FALSE in case of failure.
 */
VALUE ifx_db_num_rows(int argc, VALUE *argv, VALUE self)
{
  VALUE stmt    = Qnil;
  VALUE ret_val = Qnil;
  ...
  if (!NIL_P(stmt)) {
    ...
    memset(row_count_args,'\0',sizeof(struct _ifx_db_row_count_struct));

    row_count_args->stmt_res  =  stmt_res;
    row_count_args->count     =  0;

    #ifdef UNICODE_SUPPORT_VERSION      
	  ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_SQLRowCount_helper, row_count_args,
                     (void *)_ruby_ifx_db_Statement_level_UBF, stmt_res );
	  rc = row_count_args->rc;
    #else
      rc = _ruby_ifx_db_SQLRowCount_helper( row_count_args );
    #endif
...
/*  static int _ruby_ifx_db_get_column_by_name(stmt_handle *stmt_res, VALUE column, int release_gil)
  */
static int _ruby_ifx_db_get_column_by_name(stmt_handle *stmt_res, VALUE column, int release_gil)
{
  int rc;
  ...
  if ( stmt_res->column_info == NULL ) {
    if ( release_gil == 1 ) {
      ...
      #ifdef UNICODE_SUPPORT_VERSION        
		ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_get_result_set_info, stmt_res,
                      (void *)_ruby_ifx_db_Statement_level_UBF, stmt_res );
		rc = stmt_res->rc;
      #else
        rc = _ruby_ifx_db_get_result_set_info( stmt_res );
      #endif
...
#ifdef UNICODE_SUPPORT_VERSION
    col_name = _ruby_ifx_db_export_str_to_utf16( column );
#else
...
#ifdef UNICODE_SUPPORT_VERSION
    if ( rb_str_equal(_ruby_ifx_db_export_sqlwchar_to_utf16_rstr(stmt_res->column_info[index].name, stmt_res->column_info[index].name_length * sizeof(SQLWCHAR)), _ruby_ifx_db_export_str_to_utf16(col_name) ) ) {
#else
 ...
 */
VALUE ifx_db_field_name(int argc, VALUE *argv, VALUE self)
{
  VALUE stmt            =  Qnil;
  VALUE column          =  Qnil;
  stmt_handle* stmt_res =  NULL;
  ...
  col = _ruby_ifx_db_get_column_by_name(stmt_res,column, 1);

  if ( col < 0 ) {
    return Qfalse;
  }
#ifdef UNICODE_SUPPORT_VERSION
  return _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(stmt_res->column_info[col].name, stmt_res->column_info[col].name_length * sizeof(SQLWCHAR));
#else
 ...
 */
VALUE ifx_db_field_display_size(int argc, VALUE *argv, VALUE self)
{
  VALUE          stmt            =  Qnil;
  VALUE          column          =  Qnil;
  VALUE          ret_val         =  Qnil;
  ...
  if ( col < 0 ) {
    ...
    memset( colattr_args,'\0',sizeof(struct _ifx_db_col_attr_struct) );
    ...
    #ifdef UNICODE_SUPPORT_VERSION      
	  ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_SQLColAttributes_helper, colattr_args,
                     (void *)_ruby_ifx_db_Statement_level_UBF, stmt_res );
	  rc = colattr_args->rc;
    #else
      rc = _ruby_ifx_db_SQLColAttributes_helper( colattr_args );
    #endif
 ...
 */
VALUE ifx_db_field_num(int argc, VALUE *argv, VALUE self)
{
  VALUE        stmt         =  Qnil;
  VALUE        column       =  Qnil;
  stmt_handle  *stmt_res    =  NULL;
 ...
 */
VALUE ifx_db_field_precision(int argc, VALUE *argv, VALUE self)
{
  VALUE        stmt       =  Qnil;
  VALUE        column     =  Qnil;
  stmt_handle  *stmt_res  =  NULL;
 ...
 */
VALUE ifx_db_field_scale(int argc, VALUE *argv, VALUE self)
{
  VALUE        stmt       =  Qnil;
  VALUE        column     =  Qnil;
  stmt_handle  *stmt_res  =  NULL;
 ...
 */
VALUE ifx_db_field_type(int argc, VALUE *argv, VALUE self)
{
  VALUE        stmt       =  Qnil;
  VALUE        column     =  Qnil;
  stmt_handle  *stmt_res  =  NULL;
...
#ifdef UNICODE_SUPPORT_VERSION
  return _ruby_ifx_db_export_char_to_utf8_rstr(str_val);
#else
 ...
 * returns FALSE. 
 */
VALUE ifx_db_field_width(int argc, VALUE *argv, VALUE self)
{
  VALUE stmt    =  Qnil;
  VALUE column  =  Qnil;
  VALUE ret_val =  Qnil;
 ...
 */
VALUE ifx_db_cursor_type(int argc, VALUE *argv, VALUE self)
{
  VALUE stmt = Qnil;
  stmt_handle *stmt_res = NULL;
 ...
 * Returns TRUE on success or FALSE on failure. 
 */
VALUE ifx_db_rollback(int argc, VALUE *argv, VALUE self)
{
  VALUE connection = Qnil;
  conn_handle *conn_res;
  int rc;
 ...
 *
 */
VALUE ifx_db_free_stmt(int argc, VALUE *argv, VALUE self)
{
  VALUE        stmt         =  Qnil;
  stmt_handle  *stmt_res    =  NULL;
  ...
  if (!NIL_P(stmt)) {
    ...
    _ruby_ifx_db_free_stmt_handle_and_resources( stmt_res );
...
/*  static RETCODE _ruby_ifx_db_get_data(stmt_handle *stmt_res, int col_num, short ctype, void *buff, int in_length, SQLINTEGER *out_length) */
static RETCODE _ruby_ifx_db_get_data(stmt_handle *stmt_res, int col_num, short ctype, void *buff, int in_length, SQLINTEGER *out_length)
{
  ...
  memset(getData_args,'\0',sizeof(struct _ifx_db_get_data_args_struct));
  ...
  rc = _ruby_ifx_db_SQLGetData_helper( getData_args );
...
/* {{{ static RETCODE _ruby_ifx_db_get_length(stmt_handle* stmt_res, SQLUSMALLINT col_num, SQLINTEGER *sLength) */
static RETCODE _ruby_ifx_db_get_length(stmt_handle* stmt_res, SQLUSMALLINT col_num, SQLINTEGER *sLength)
{
  RETCODE          rc               = SQL_SUCCESS;
  SQLHANDLE        new_hstmt;
  ...
  memset(getLength_args,'\0',sizeof(struct _ifx_db_get_data_length_struct));
  ...
  rc = _ruby_ifx_db_SQLGetLength_helper( getLength_args );

  if ( rc == SQL_ERROR ) {
    _ruby_ifx_db_check_sql_errors( stmt_res, DB_STMT, (SQLHSTMT)new_hstmt, SQL_HANDLE_STMT, rc, 1, NULL, NULL, -1, 1, 0 );
  }
...
/* {{{ static RETCODE _ruby_ifx_db_get_data2(stmt_handle *stmt_res, int col_num, short ctype, void *buff, int in_length, SQLINTEGER *out_length) */
static RETCODE _ruby_ifx_db_get_data2(stmt_handle *stmt_res, SQLUSMALLINT col_num, SQLSMALLINT ctype, SQLPOINTER buff, SQLLEN read_length, SQLLEN buff_length, SQLINTEGER *out_length)
{
  RETCODE rc = SQL_SUCCESS;
  SQLHANDLE new_hstmt;
  ...
  memset(getSubString_args,'\0', sizeof(struct _ifx_db_get_data_subString_struct) );
  ...
  rc = _ruby_ifx_db_SQLGetSubString_helper( getSubString_args );
...
int isNullLOB(VALUE *return_value,int i,stmt_handle *stmt_res,int op)
{
    ...
    if (stmt_res->column_info[i].loc_ind == SQL_NULL_DATA) {
        if ( op & FETCH_ASSOC ) {
#ifdef UNICODE_SUPPORT_VERSION
          colName = _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(stmt_res->column_info[i].name, stmt_res->column_info[i].name_length * sizeof(SQLWCHAR) );
#else
...
/*
  static VALUE _ruby_ifx_db_result_helper(ifx_db_result_args *data)
...
static VALUE _ruby_ifx_db_result_helper(ifx_db_result_args *data) {
  ...
  if(TYPE(column) == T_STRING) {
    col_num = _ruby_ifx_db_get_column_by_name(stmt_res, column, 0);
  } else {
  ...
  /* get column header info*/
  if ( stmt_res->column_info == NULL ) {
    if (_ruby_ifx_db_get_result_set_info(stmt_res)<0) {
      if( stmt_res != NULL && stmt_res->ruby_stmt_err_msg != NULL ) {
#ifdef UNICODE_SUPPORT_VERSION
        *(data->error) = rb_str_concat( _ruby_ifx_db_export_char_to_utf8_rstr("Column information cannot be retrieved: "),
                           _ruby_ifx_db_export_sqlwchar_to_utf8_rstr( stmt_res->ruby_stmt_err_msg,
                                     stmt_res->ruby_stmt_err_msg_len)
                         );
#else
...
#ifdef UNICODE_SUPPORT_VERSION
        *(data->error) = _ruby_ifx_db_export_char_to_utf8_rstr("Column information cannot be retrieved: <error message could not be retrieved>");
#else
...
#ifdef UNICODE_SUPPORT_VERSION
    *(data->error) = _ruby_ifx_db_export_char_to_utf8_rstr("Column ordinal out of range" );
#else
  ...
  switch(column_type) {
...
#ifdef UNICODE_SUPPORT_VERSION
      rc = _ruby_ifx_db_get_data(stmt_res, col_num+1, SQL_C_WCHAR, out_ptr, in_length * sizeof(SQLWCHAR) , &out_length);
#else
      rc = _ruby_ifx_db_get_data(stmt_res, col_num+1, SQL_C_CHAR, out_ptr, in_length, &out_length);
#endif
...
#ifdef UNICODE_SUPPORT_VERSION
        return_value = _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(out_ptr, out_length);
#else
    ...
    case SQL_SMALLINT:
    case SQL_INTEGER:
      rc = _ruby_ifx_db_get_data(stmt_res, col_num+1, SQL_C_LONG, &long_val, sizeof(long_val), &out_length);
      if ( rc == SQL_ERROR ) {
		data->return_value = Qfalse;
        return Qfalse;
      }
    ...
    case SQL_FLOAT:
    case SQL_DOUBLE:
      rc = _ruby_ifx_db_get_data(stmt_res, col_num+1, SQL_C_DOUBLE, &double_val, sizeof(double_val), &out_length);
      if ( rc == SQL_ERROR ) {
		data->return_value = Qfalse;
        return Qfalse;
      }
    ...
    case SQL_CLOB:
      ...
      rc = _ruby_ifx_db_get_length(stmt_res, col_num+1, &in_length);
...
#ifdef UNICODE_SUPPORT_VERSION
      rc = _ruby_ifx_db_get_data2(stmt_res, col_num+1, SQL_C_WCHAR, out_ptr, in_length, (in_length + 1) * sizeof(SQLWCHAR) , &out_length);
#else
      rc = _ruby_ifx_db_get_data2(stmt_res, col_num+1, SQL_C_CHAR, (void*)out_ptr, in_length, in_length+1, &out_length);
#endif
    ...
    case SQL_VARBINARY:
      ...
      switch (stmt_res->s_bin_mode) {
        ...
        case BINARY:
          ...
          rc = _ruby_ifx_db_get_data2(stmt_res, col_num+1, lob_bind_type, out_ptr, in_length, in_length, &out_length);
    ...
    case SQL_XML:
      ...
      rc      =  _ruby_ifx_db_get_data(stmt_res, col_num+1, SQL_C_BINARY, NULL, 0, (SQLINTEGER *)&in_length);
      ...
      rc = _ruby_ifx_db_get_data(stmt_res, col_num+1, SQL_C_BINARY, out_ptr, in_length, &out_length);
...
#ifdef UNICODE_SUPPORT_VERSION
      return_value = _ruby_ifx_db_export_sqlchar_to_utf8_rstr(out_ptr, out_length);
#else
      return_value = rb_str_new((char*)out_ptr,out_length);
#endif
 ...
 */
VALUE ifx_db_result(int argc, VALUE *argv, VALUE self)
{
  ifx_db_result_args *result_args  =  NULL;
  VALUE ret_val                    =  Qfalse;
  VALUE stmt                       =  Qnil;

  VALUE error  =  Qnil;

  result_args = ALLOC( ifx_db_result_args );
  memset(result_args,'\0',sizeof(struct _ifx_db_result_args_struct));
  ...
  if ( !NIL_P( stmt ) ) {
    ...
    #ifdef UNICODE_SUPPORT_VERSION      
	  ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_result_helper, result_args,
                          (void *)_ruby_ifx_db_Statement_level_UBF, result_args->stmt_res );
	  ret_val = result_args->return_value;
    #else
      ret_val = _ruby_ifx_db_result_helper( result_args );
    #endif
...
/*  static VALUE _ruby_ifx_db_bind_fetch_helper(ifx_db_fetch_helper_args *data)
*/
static VALUE _ruby_ifx_db_bind_fetch_helper(ifx_db_fetch_helper_args *data)
{
  int         rc             = -1;
  ...
  _ruby_ifx_db_init_error_info(stmt_res);
  ...
  if ( stmt_res->row_data == NULL ) {
    rc = _ruby_ifx_db_bind_column_helper(stmt_res);
    if ( rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO ) {
      if( stmt_res != NULL && stmt_res->ruby_stmt_err_msg != NULL ) {
#ifdef UNICODE_SUPPORT_VERSION
        *(data->error) = rb_str_concat( _ruby_ifx_db_export_char_to_utf8_rstr("Column binding cannot be done: "),
                           _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(stmt_res->ruby_stmt_err_msg,
                                     stmt_res->ruby_stmt_err_msg_len)
                         );
#else
...
#ifdef UNICODE_SUPPORT_VERSION
        *(data->error) = _ruby_ifx_db_export_char_to_utf8_rstr("Column binding cannot be done: <error message could not be retrieved>");
#else
  ...
  if (data->arg_count == 2 && row_number > 0) {
    ...
    if (is_systemi) {
      ...
      rc = _ruby_ifx_db_SQLFetchScroll_helper( fetch_args );
...
#ifdef UNICODE_SUPPORT_VERSION
    *(data->error) = _ruby_ifx_db_export_char_to_utf8_rstr("Requested row number must be a positive value");
#else
  ...
  if (rc == SQL_NO_DATA_FOUND) {
    ...
    if( stmt_res != NULL && stmt_res->ruby_stmt_err_msg != NULL ) {
#ifdef UNICODE_SUPPORT_VERSION
      *(data->error) = rb_str_concat( _ruby_ifx_db_export_char_to_utf8_rstr("Fetch Failure: "),
                         _ruby_ifx_db_export_sqlwchar_to_utf8_rstr( stmt_res->ruby_stmt_err_msg,
                                      stmt_res->ruby_stmt_err_msg_len )
                       );
#else
...
#ifdef UNICODE_SUPPORT_VERSION
      *(data->error) = _ruby_ifx_db_export_char_to_utf8_rstr("Fetch Failure: <error message could not be retrieved>");
#else
    ...
    if (out_length == SQL_NULL_DATA) {
      ...
      switch(column_type) {
        ...
        case SQL_WLONGVARCHAR:
          if ( op & FETCH_ASSOC ) {
#ifdef UNICODE_SUPPORT_VERSION
            rb_hash_aset(return_value, colName, _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(row_data->str_val, out_length ));
#else
          ...
          if ( op == FETCH_INDEX ) {
#ifdef UNICODE_SUPPORT_VERSION
            rb_ary_store(return_value, i, _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(row_data->str_val, out_length ));
#else
            rb_ary_store(return_value, i, rb_str_new((char *)row_data->str_val, out_length));
#endif
          } else if ( op == FETCH_BOTH ) {
#ifdef UNICODE_SUPPORT_VERSION
            rb_hash_aset(return_value, INT2NUM(i), _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(row_data->str_val, out_length ));
#else
          ...
          out_length = 0;
          rc = _ruby_ifx_db_get_data(stmt_res, i+1, SQL_C_CHAR, out_ptr, tmp_length+1, &out_length);
          if ( rc == SQL_ERROR ) {
            ruby_xfree( out_ptr );
            out_ptr = NULL;
			data->return_value = Qfalse;
        ...
        /*Check if the data value in the column is null*/
          ...
          out_ptr  =  NULL;
          rc       =  _ruby_ifx_db_get_length(stmt_res, i+1, &tmp_length);
          ...
          if (tmp_length == SQL_NULL_DATA) {
            ...
            switch (stmt_res->s_bin_mode) {
              ...
              case BINARY:
                ...
                rc = _ruby_ifx_db_get_data2(stmt_res, i+1, lob_bind_type, (char *)out_ptr, tmp_length, tmp_length, &out_length);
                if (rc == SQL_ERROR) {
                  ruby_xfree( out_ptr );
                  out_ptr = NULL;
                  out_length = 0;
        ...
        /*Check if the data value in the column is null*/
          ...
          out_ptr =  NULL;
          rc      =  _ruby_ifx_db_get_data(stmt_res, i+1, SQL_C_BINARY, NULL, 0, (SQLINTEGER *)&tmp_length);

          if ( rc == SQL_ERROR ) {
            if( stmt_res != NULL && stmt_res->ruby_stmt_err_msg != NULL ) {
#ifdef UNICODE_SUPPORT_VERSION
              *(data->error) = rb_str_concat( _ruby_ifx_db_export_char_to_utf8_rstr("Failed to Determine XML Size: "),
                                 _ruby_ifx_db_export_sqlwchar_to_utf8_rstr( stmt_res->ruby_stmt_err_msg,
                                           stmt_res->ruby_stmt_err_msg_len)
                               );
#else
...
#ifdef UNICODE_SUPPORT_VERSION
              *(data->error) = _ruby_ifx_db_export_char_to_utf8_rstr("Failed to Determine XML Size: <error message could not be retrieved>");
#else
          ...
          if (tmp_length == SQL_NULL_DATA) {
            ...
            rc = _ruby_ifx_db_get_data(stmt_res, i+1, SQL_C_BINARY, out_ptr, tmp_length, &out_length);
            if (rc == SQL_ERROR) {
              ruby_xfree( out_ptr );
              out_ptr = NULL;
			  data->return_value = Qfalse;
            ...
            if ( op & FETCH_ASSOC ) {
#ifdef UNICODE_SUPPORT_VERSION
              rb_hash_aset(return_value, colName, _ruby_ifx_db_export_sqlchar_to_utf8_rstr( (SQLCHAR *) out_ptr, out_length));
#else
              rb_hash_aset(return_value, colName, rb_str_new((char *)out_ptr, out_length));
#endif
            }
            if ( op == FETCH_INDEX ) {
#ifdef UNICODE_SUPPORT_VERSION
              rb_ary_store(return_value, i, _ruby_ifx_db_export_sqlchar_to_utf8_rstr( (SQLCHAR *) out_ptr, out_length));
#else
              rb_ary_store(return_value, i, rb_str_new((char *)out_ptr, out_length));
#endif
            } else if ( op == FETCH_BOTH ) {
#ifdef UNICODE_SUPPORT_VERSION
              rb_hash_aset(return_value, INT2NUM(i), _ruby_ifx_db_export_sqlchar_to_utf8_rstr( (SQLCHAR *) out_ptr, out_length));
#else
              rb_hash_aset(return_value, INT2NUM(i), rb_str_new((char *)out_ptr, out_length));
#endif
            }
        ...
        /*Check if the data value in the column is null*/
          ...
          if (tmp_length == SQL_NULL_DATA) {
...
#ifdef UNICODE_SUPPORT_VERSION
            rc = _ruby_ifx_db_get_data2(stmt_res, i+1, SQL_C_WCHAR, out_ptr, tmp_length , (tmp_length + 1) * sizeof(SQLWCHAR) , &out_length);
#else
            rc = _ruby_ifx_db_get_data2(stmt_res, i+1, SQL_C_CHAR, out_ptr, tmp_length, tmp_length+1, &out_length);
#endif
            if (rc == SQL_ERROR) {
              ruby_xfree( out_ptr );
              out_ptr =  NULL;
            ...
            if ( op & FETCH_ASSOC ) {
#ifdef UNICODE_SUPPORT_VERSION
              rb_hash_aset(return_value, colName, _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(out_ptr, out_length));
#else
              rb_hash_aset(return_value, colName, rb_str_new((char*)out_ptr, out_length));
#endif
            }
            if ( op == FETCH_INDEX ) {
#ifdef UNICODE_SUPPORT_VERSION
              rb_ary_store(return_value, i, _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(out_ptr, out_length));
#else
              rb_ary_store(return_value, i, rb_str_new((char*)out_ptr, out_length));
#endif
            } else if ( op == FETCH_BOTH ) {
#ifdef UNICODE_SUPPORT_VERSION
              rb_hash_aset(return_value, INT2NUM(i), _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(out_ptr, out_length));
#else
              rb_hash_aset(return_value, INT2NUM(i), rb_str_new((char*)out_ptr, out_length));
#endif
            }
...
/*
  static int _ruby_ifx_db_fetch_row_helper( ifx_db_fetch_helper_args *data)
*/
static int _ruby_ifx_db_fetch_row_helper( ifx_db_fetch_helper_args *data) {
  stmt_handle *stmt_res  = data->stmt_res;
  SQLLEN      row_number = 0;
 ...
 */
VALUE ifx_db_fetch_row(int argc, VALUE *argv, VALUE self)
{
  ...
  ifx_db_fetch_helper_args *helper_args = NULL;
  ...
  helper_args = ALLOC( ifx_db_fetch_helper_args );
  memset(helper_args,'\0',sizeof(struct _ifx_db_fetch_helper_struct));
  ...
  #ifdef UNICODE_SUPPORT_VERSION    
	ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_fetch_row_helper, helper_args,
                        (void *)_ruby_ifx_db_Statement_level_UBF, stmt_res );
	ret_val = helper_args->return_value;
  #else
    ret_val = _ruby_ifx_db_fetch_row_helper( helper_args );
  #endif
 ...
 * 
 */
VALUE ifx_db_result_cols(int argc, VALUE *argv, VALUE self) {
  VALUE stmt             =  Qnil;
  VALUE ret_val          =  Qnil;
  ...
  if ( stmt_res->column_info == NULL ) {
    if (_ruby_ifx_db_get_result_set_info(stmt_res)<0) {
      if( stmt_res != NULL && stmt_res->ruby_stmt_err_msg != NULL ) {
#ifdef UNICODE_SUPPORT_VERSION
        error = rb_str_concat( _ruby_ifx_db_export_char_to_utf8_rstr("Column information cannot be retrieved: "),
                           _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(stmt_res->ruby_stmt_err_msg,
                                     stmt_res->ruby_stmt_err_msg_len)
                         );
#else
...
#ifdef UNICODE_SUPPORT_VERSION
        error = _ruby_ifx_db_export_char_to_utf8_rstr("Column information cannot be retrieved: <error message could not be retrieved>");
#else
...
#ifdef UNICODE_SUPPORT_VERSION
    colName = _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(stmt_res->column_info[index].name, stmt_res->column_info[index].name_length * sizeof(SQLWCHAR) );
#else
 ...
 */
VALUE ifx_db_fetch_assoc(int argc, VALUE *argv, VALUE self) {
  VALUE row_number       =  Qnil;
  VALUE stmt             =  Qnil;
  VALUE ret_val          =  Qnil;
  ...
  #ifdef UNICODE_SUPPORT_VERSION    
	ibm_Ruby_Thread_Call ( (void *)_ruby_ifx_db_bind_fetch_helper, helper_args,
                        (void *)_ruby_ifx_db_Statement_level_UBF, stmt_res );
	ret_val = helper_args->return_value;
						
  #else
    ret_val = _ruby_ifx_db_bind_fetch_helper( helper_args );
  #endif
 ...
 */
VALUE ifx_db_fetch_object(int argc, VALUE *argv, VALUE self)
{
  VALUE     row_number   =  Qnil;
  VALUE     stmt         =  Qnil;
  VALUE     ret_val      =  Qnil;
  ...
  #else
    row_res->hash = _ruby_ifx_db_bind_fetch_helper( helper_args );
  #endif
  ...
  if (RTEST(row_res->hash)) {
    return Data_Wrap_Struct(le_row_struct,
        _ruby_ifx_db_mark_row_struct, _ruby_ifx_db_free_row_struct,
        row_res);
  } else {
    ruby_xfree( row_res );
    row_res = NULL;
 ...
 * or if the row requested by row_number does not exist in the result set. 
 */
VALUE ifx_db_fetch_array(int argc, VALUE *argv, VALUE self)
{
  VALUE row_number       =  Qnil;
  VALUE stmt             =  Qnil;
  VALUE ret_val          =  Qnil;
 ...
 */
VALUE ifx_db_fetch_both(int argc, VALUE *argv, VALUE self)
{
  VALUE row_number       =  Qnil;
  VALUE stmt             =  Qnil;
  VALUE ret_val          =  Qnil;
 ...
 * Returns TRUE on success or FALSE on failure
 */
VALUE ifx_db_set_option(int argc, VALUE *argv, VALUE self)
{
  VALUE conn_or_stmt = Qnil;
  VALUE r_options;
  VALUE r_type;
  ...
  if (!NIL_P(conn_or_stmt)) {
    if ( type == 1 ) {
      ...
      if ( !NIL_P(r_options) ) {
        rc = _ruby_ifx_db_parse_options( r_options, SQL_HANDLE_DBC, conn_res, &error );
        if (rc == SQL_ERROR) {
          rb_warn( RSTRING_PTR(error) );
          return Qfalse;
        }
      ...
      if ( !NIL_P(r_options) ) {
        rc = _ruby_ifx_db_parse_options( r_options, SQL_HANDLE_STMT, stmt_res, &error );
        if (rc == SQL_ERROR) {
          rb_warn( RSTRING_PTR(error) );
          return Qfalse;
        }
...
*/
static VALUE ifx_db_server_info_helper( get_info_args *getInfo_args ) {	
  conn_handle *conn_res = NULL;
...
#ifdef UNICODE_SUPPORT_VERSION
    rb_iv_set(return_value, "@DBMS_NAME", _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(buffer255, out_length));
#else
...
#ifdef UNICODE_SUPPORT_VERSION
    rb_iv_set(return_value, "@DBMS_VER", _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(buffer11, out_length));
#else
...
#ifdef UNICODE_SUPPORT_VERSION
   rb_iv_set(return_value, "@DB_NAME", _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(buffer255, out_length));
#else
...
#ifdef UNICODE_SUPPORT_VERSION
    rb_iv_set(return_value, "@INST_NAME", _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(buffer255, out_length));
#else
...
#ifdef UNICODE_SUPPORT_VERSION
    rb_iv_set(return_value, "@SPECIAL_CHARS", _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(buffer255, out_length));
#else
...
#ifdef UNICODE_SUPPORT_VERSION
    keywordsStr   =  _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(buffer3k, out_length);
    keywordsArray =  rb_str_split(keywordsStr, RSTRING_PTR(_ruby_ifx_db_export_char_to_utf8_rstr(",")) );
#else
...
#ifdef UNICODE_SUPPORT_VERSION
     dft_isolation = _ruby_ifx_db_export_char_to_utf8_rstr("UR");
#else
...
#ifdef UNICODE_SUPPORT_VERSION
      dft_isolation = _ruby_ifx_db_export_char_to_utf8_rstr("CS");
#else
...
#ifdef UNICODE_SUPPORT_VERSION
      dft_isolation = _ruby_ifx_db_export_char_to_utf8_rstr("RS");
#else
...
#ifdef UNICODE_SUPPORT_VERSION
      dft_isolation = _ruby_ifx_db_export_char_to_utf8_rstr("RR");
#else
...
#ifdef UNICODE_SUPPORT_VERSION
      dft_isolation = _ruby_ifx_db_export_char_to_utf8_rstr("NC");
#else
  ...
  if ( rc == SQL_ERROR ) {
    ...
    if( bitmask & SQL_TXN_READ_UNCOMMITTED ) {
#ifdef UNICODE_SUPPORT_VERSION
     rb_ary_push(array, _ruby_ifx_db_export_char_to_utf8_rstr("UR"));
#else
    ...
    if( bitmask & SQL_TXN_READ_COMMITTED ) {
#ifdef UNICODE_SUPPORT_VERSION
      rb_ary_push(array, _ruby_ifx_db_export_char_to_utf8_rstr("CS"));
#else
    ...
    if( bitmask & SQL_TXN_REPEATABLE_READ ) {
#ifdef UNICODE_SUPPORT_VERSION
      rb_ary_push(array, _ruby_ifx_db_export_char_to_utf8_rstr("RS"));
#else
    ...
    if( bitmask & SQL_TXN_SERIALIZABLE ) {
#ifdef UNICODE_SUPPORT_VERSION
      rb_ary_push(array, _ruby_ifx_db_export_char_to_utf8_rstr("RR"));
#else
    ...
    if( bitmask & SQL_TXN_NOCOMMIT ) {
#ifdef UNICODE_SUPPORT_VERSION
      rb_ary_push(array, _ruby_ifx_db_export_char_to_utf8_rstr("NC"));
#else
...
#ifdef UNICODE_SUPPORT_VERSION
        conformance = _ruby_ifx_db_export_char_to_utf8_rstr("ENTRY");
#else
...
#ifdef UNICODE_SUPPORT_VERSION
        conformance = _ruby_ifx_db_export_char_to_utf8_rstr("FIPS127");
#else
...
#ifdef UNICODE_SUPPORT_VERSION
        conformance = _ruby_ifx_db_export_char_to_utf8_rstr("FULL");
#else
...
#ifdef UNICODE_SUPPORT_VERSION
        conformance = _ruby_ifx_db_export_char_to_utf8_rstr("INTERMEDIATE");
#else
...
#ifdef UNICODE_SUPPORT_VERSION
    if( rb_str_equal(_ruby_ifx_db_export_sqlwchar_to_utf8_rstr(buffer11, out_length), _ruby_ifx_db_export_char_to_utf8_rstr("Y")) ) {
#else
...
#ifdef UNICODE_SUPPORT_VERSION
    rb_iv_set(return_value, "@IDENTIFIER_QUOTE_CHAR", _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(buffer11, out_length));
#else
 ...
 */
VALUE ifx_db_server_info(int argc, VALUE *argv, VALUE self)
{
  VALUE       connection  =  Qnil;
  conn_handle *conn_res   =  NULL;
  ...
  if (!NIL_P(connection)) {
    ...
    #ifdef UNICODE_SUPPORT_VERSION
	  ibm_Ruby_Thread_Call ( (void *)ifx_db_server_info_helper, getInfo_args,
                                 (void *)_ruby_ifx_db_Connection_level_UBF, NULL);
	  return_value  = getInfo_args->return_value;
    #else
      return_value = ifx_db_server_info_helper( getInfo_args );
    #endif
...
*/
static VALUE ifx_db_client_info_helper( get_info_args *getInfo_args) {
  conn_handle *conn_res = NULL;
...
#ifdef UNICODE_SUPPORT_VERSION
    rb_iv_set(return_value, "@DRIVER_NAME", _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(buffer255, out_length));
#else
...
#ifdef UNICODE_SUPPORT_VERSION
    rb_iv_set(return_value, "@DRIVER_VER", _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(buffer255, out_length));
#else
...
#ifdef UNICODE_SUPPORT_VERSION
    rb_iv_set(return_value, "@DATA_SOURCE_NAME", _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(buffer255, out_length));
#else
...
#ifdef UNICODE_SUPPORT_VERSION
    rb_iv_set(return_value, "@DRIVER_ODBC_VER", _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(buffer255, out_length));
#else
...
#ifdef UNICODE_SUPPORT_VERSION
    rb_iv_set(return_value, "@ODBC_VER", _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(buffer255, out_length));
#else
  ...
  if ( rc == SQL_ERROR ) {
    ...
    switch (bufferint16) {
...
#ifdef UNICODE_SUPPORT_VERSION
       conformance = _ruby_ifx_db_export_char_to_utf8_rstr("MINIMUM");
#else
      ...
      case SQL_OSC_CORE:
#ifdef UNICODE_SUPPORT_VERSION
       conformance = _ruby_ifx_db_export_char_to_utf8_rstr("CORE");
#else
...
#ifdef UNICODE_SUPPORT_VERSION
       conformance = _ruby_ifx_db_export_char_to_utf8_rstr("EXTENDED");
#else
 ...
 * Returns an object on a successful call. Returns FALSE on failure. 
 */
VALUE ifx_db_client_info(int argc, VALUE *argv, VALUE self)
{
  VALUE connection = Qnil;
  conn_handle *conn_res;
  ...
  if (!NIL_P(connection)) {
    ...
    #ifdef UNICODE_SUPPORT_VERSION      
	  ibm_Ruby_Thread_Call ( (void *)ifx_db_client_info_helper, getInfo_args,
                               (void *)_ruby_ifx_db_Connection_level_UBF, NULL);
	  return_value = getInfo_args->return_value;
							   
    #else
      return_value = ifx_db_client_info_helper( getInfo_args );
    #endif
 ...
 */
VALUE ifx_db_active(int argc, VALUE *argv, VALUE self)
{
  VALUE connection = Qnil;
  int rc;
  conn_handle *conn_res;
 ...
 */
VALUE ifx_db_get_option(int argc, VALUE *argv, VALUE self)
{
  VALUE conn_or_stmt        =  Qnil;
  VALUE option              =  Qnil;
  VALUE r_type              =  Qnil;
...
#ifdef UNICODE_SUPPORT_VERSION
          ret_val = _ruby_ifx_db_export_sqlwchar_to_utf8_rstr(value, out_length);
#else
    ...
    /* At this point we know we are to retreive a statement option */
      ...
      if (!NIL_P(option)) {
        ...
        if (op_integer == SQL_ATTR_CURSOR_TYPE) {
          ...
          get_handleAttr_args->out_length  =  NULL;
          rc = _ruby_ifx_db_SQLGetStmtAttr_helper( get_handleAttr_args );
 ...
 * Returns FALSE in case of failure.
 */
VALUE ifx_db_get_last_serial_value(int argc, VALUE *argv, VALUE self)
{
  VALUE   stmt    = Qnil;
  VALUE   ret_val = Qnil;
  ...
  if (!NIL_P(stmt)) {
    ...
    if ( rc == SQL_ERROR ) {
      _ruby_ifx_db_check_sql_errors( stmt_res, DB_STMT, (SQLHSTMT)stmt_res->hstmt, SQL_HANDLE_STMT, rc, 1, NULL, NULL, -1, 1, 1 );
      return Qfalse;
    }
